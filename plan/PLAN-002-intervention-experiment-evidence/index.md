---
type: index
title: "PLAN-002 — Intervention-experiment evidence implementation"
description: "Contents of the PLAN-002 bundle."
---

# PLAN-002 — Intervention-experiment evidence implementation

## Contents

- [PLAN-002](./plan.md) — Plan overview, dependency graph, tracks, and gates.
- [TASK-006](./tasks/TASK-006-record-contract.md) — Record schema and validator.
- [TASK-007](./tasks/TASK-007-governed-intake.md) — Governed atomic intake.
- [TASK-008](./tasks/TASK-008-claim-report.md) — Claim-centered projection.
- [TASK-009](./tasks/TASK-009-agent-eval-producer.md) — First real producer.
- [TASK-010](./tasks/TASK-010-integration-gate.md) — E2E and completion gates.
