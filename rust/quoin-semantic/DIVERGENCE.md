...
## 6. Divergence D4 — YAML, which is not a JSON Schema question at all

Found while porting, recorded because it is the same class of risk and nothing
else will catch it.

The TypeScript parses `manifest.yaml` and document frontmatter with the `yaml`
npm package, which implements **YAML 1.2**. Both `serde_yaml` (quire-rs's choice)
and `serde_norway` (this port's) are libyaml-backed and implement **YAML 1.1**.
They disagree on:

| Source             | YAML 1.2 (`yaml` npm) | YAML 1.1 (libyaml) |
| ------------------ | --------------------- | ------------------ |
| `legacy_forms: no` | string `"no"`         | boolean `false`    |
| `version: 1.10`    | number `1.10`         | number `1.1`       |
| `revision: 12:30`  | string `"12:30"`      | sexagesimal `750`  |
| `id: 0o17`         | number `15`           | string `"0o17"`    |
| `id: 017`          | number `17`           | octal `15`         |

**Blast radius here is bounded and measured as zero**: every `semantic` key is a
string from a closed vocabulary, and the schema refuses a non-string before any
of these could matter — `legacy_forms: no` fails `enum` under both readings. The
completeness reader takes only string and list-of-string frontmatter values and
drops everything else.

**It is not bounded later.** Frontmatter fields with free values (an `id`, a
`revision`, a numeric threshold) reach Stage 4's evidence store and Stage 6's
measurement, where `12:30` becoming `750` is a silently wrong recorded number.
**Flagged for Stage 4: either a YAML 1.2 reader, or a documented decision that
quoin's YAML is 1.1 and the TypeScript was wrong.** Do not let this be discovered
by a measurement.
...
# 2. Re-run parity.
cargo +1.98.1 test -p quoin-semantic --target-dir <dir>
cargo +1.98.1 test -p quoin-completeness --target-dir <dir>
```
