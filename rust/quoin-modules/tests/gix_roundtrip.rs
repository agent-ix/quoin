...
fn tc_381_290_gix_resolves_a_tag_and_extracts_the_whole_tree() {
    ...
    let resolved = resolver
        ...
        .expect("tag resolves");
    assert_eq!(resolved.sha.as_str(), commit);
    assert_eq!(resolved.requested_ref.as_deref(), Some("v1.0.0"));
    assert_eq!(
        fs::read_to_string(resolved.dir.join("manifest.yaml")).expect("root manifest"),
        "name: fixture-root\n"
    );
    assert_eq!(
        fs::read_to_string(
...
fn tc_381_291_gix_extracts_only_the_requested_subdirectory() {
    ...
    assert_eq!(
        fs::read_to_string(resolved.dir.join("manifest.yaml")).expect("subdir manifest"),
        "name: spec-objects-fixture\n",
        "the subdir's own manifest must be at the content root"
    );
    assert!(
        !resolved.dir.join("spec_objects_fixture").exists(),
