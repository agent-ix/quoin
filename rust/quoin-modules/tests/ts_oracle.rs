...
use quoin_modules::{parse_source_arg, read_module_name, to_git_url, IxHome, Source};
...
fn tc_381_232_source_validation_accepts_and_refuses_what_the_oracle_did() {
    ...
    for case in cases {
        ...
        let actual_ok = serde_json::from_value::<Source>(input.clone())
            .ok()
            .is_some_and(|s| s.validate().is_ok());
