...
//!
//! This is the crate's **one** census, modelled on
//! `quoin-quire/tests/tc_474_module_sizes.rs`. A second here would be a copy
...
///
/// Empty. The retained `src/auditor/combinatorial.ts` is 195 lines and this
/// crate is its port plus the documentation of what differs, split across six
/// modules by responsibility — the ECMAScript primitives, the error catalogue,
/// the parser, the tuple algebra, the measurement, and the crate root. Adding a
/// name here is a deliberate act that has to be argued for.
...
#[test]
fn tc_382_020_no_module_is_over_the_hard_ceiling() {
    let modules = modules();
    assert!(
...
#[test]
fn tc_382_021_the_modules_over_the_soft_ceiling_are_the_ones_named_here() {
