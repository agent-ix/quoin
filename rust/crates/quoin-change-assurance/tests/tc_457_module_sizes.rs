...
//! looking, and `model/record.rs` reached 1,078 lines that way before quoin#457
//! split it into [`quoin_change_assurance::model::record`]'s four modules. So
...
#[test]
fn tc_457_no_module_is_over_the_hard_ceiling() {
    let modules = modules();
    assert!(
        ...
        assert!(
            *lines <= HARD_CEILING,
            "{name} is {lines} lines, over the {HARD_CEILING}-line hard ceiling (quoin#464). \
             Split it on responsibility, as `model/record.rs` was split at quoin#457. Do not \
             raise the ceiling."
        );
    }
}
...
#[test]
fn tc_457_the_modules_over_the_soft_ceiling_are_the_ones_named_here() {
