...
//! redeclares `subject`, `producer` and the environment map inline — the same shapes, written twice. This module is the
//! one declaration of each, which is the plan's §13.2 criterion applied at port
//! time rather than left for a later deduplication pass.
