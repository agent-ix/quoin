...
//!
//! It covers the operations that still HAVE two implementations, which after
//! the assurance and completeness cutover (quoin#445/#447) is `core.ping`
//! alone. That is the harness working, not the harness shrinking: a domain
//! whose TypeScript has been deleted has nothing left to difference against,
//! and FR-101-AC-5 forbids keeping a non-Rust oracle alive to pretend
//! otherwise. Those domains are now gated against the real binary by their own
//! integration tests instead — see [`CASES`].
//!
//! ```text
