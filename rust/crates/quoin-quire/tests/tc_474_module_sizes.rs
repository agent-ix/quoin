...
//!
//! This is the crate's **one** census — `quoin-jsonschema` carries the same
//! test for its own tree, and a second one here would be a copy rather than a
//! measurement. It arrived with quoin#474 because that is when this crate first
//! gained a module; the two entries below predate it and are recorded, not
//! introduced, by it.
//!
...
/// The modules currently over [`SOFT_CEILING`], each with what it holds.
///
/// Both predate quoin#474. `error.rs` is the crate's single error enum and its
/// code table, and it grew by the two variants quoin#474 added; splitting an
/// error taxonomy across files to satisfy a line count would scatter the one
/// place a reader looks up a `QQ-` code. `properties.rs` is untouched by this
/// ticket. Neither is near the hard ceiling, and neither is a licence to add a
/// third: a new name here is a deliberate act that has to be argued for.
const OVER_SOFT_CEILING: &[&str] = &["error.rs", "properties.rs"];
...
#[test]
fn tc_474_030_no_module_is_over_the_hard_ceiling() {
    let modules = modules();
    assert!(
...
#[test]
fn tc_474_031_the_modules_over_the_soft_ceiling_are_the_ones_named_here() {
