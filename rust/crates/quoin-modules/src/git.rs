...
impl Deadline {
    ...
    ///
    /// # Errors
    /// [`ModulesError::FetchDeadlineUnavailable`] if the watchdog thread cannot
    /// be spawned. The failure is propagated rather than swallowed: without the
    /// watchdog the blocking fetch below has no wall-clock bound at all, and a
    /// caller that believes it has one would wait forever.
    fn start(budget: Duration) -> Result<Self, ModulesError> {
        ...
        let watchdog = {
            ...
            std::thread::Builder::new()
                ...
                .map_err(|source| ModulesError::FetchDeadlineUnavailable {
    ...
    /// Whether the watchdog is what stopped the operation.
    ///
    /// Only the interrupt flag answers this. Comparing elapsed time against the
    /// budget would also answer "yes" for a transport that failed on its own
    /// after the budget had passed, which relabels a real error as a timeout
    /// and throws the real error away.
...
fn fetch_into(
    ...
    if let Revision::Sha(sha) = revision
        && repo.rev_parse_single(sha).is_ok()
    {
        return Ok(repo);
    }

    let deadline = Deadline::start(limits.fetch_budget)?;
    // The third refspec is not decoration: `refs/heads/*` creates no ref named
    // `HEAD`, so without it `refs/remotes/origin/HEAD` never exists and every
    // unpinned source — the default — fails to resolve. This is the same
    // refspec `gix`'s own clone adds for exactly this reason.
    let refspecs: &[&str] = &[
    ...
    match outcome {
        Ok(_) => Ok(repo),
        // A fetch stopped by the watchdog surfaces as a transport error. Only
        // the interrupt flag says the watchdog was the cause, and the symptom
        // is carried along rather than dropped: a fetch that failed for its own
        // reasons a moment after the budget elapsed must keep its own message.
        Err(err) if deadline.timed_out() => Err(ModulesError::GitTimeout {
            url: url.to_owned(),
...
mod tests {
    ...
    fn tc_381_252_deadline_raises_the_interrupt_flag_and_joins() {
        let deadline = Deadline::start(Duration::from_millis(50)).expect("watchdog spawns");
