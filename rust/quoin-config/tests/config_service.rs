...
fn service<'a>(f: &'a Fixture) -> ConfigService<'a, QuoinConfig> {
...
fn tc_381_120_set_key_writes_only_the_named_key() {
    ...
    assert!(text.starts_with("# Managed by quoin"), "header: {text:?}");
...
/// Trace: FR-027-AC-5
#[test]
fn tc_381_125_a_write_refuses_an_unparsable_file_and_reset_clears_it() {
    let f = fixture();
    let svc = service(&f);
    fs::create_dir_all(f.user_config.parent().expect("has a parent")).expect("config dir");
    ...
    // A write is not a read: `set` merges into what is on disk, so it refuses
    // rather than silently discarding a file it cannot understand. Matching the
    // oracle, which lets the same parse error escape `set`.
    let err = svc
        .set_key("org", "acme", &mut log)
        .expect_err("set must not overwrite an unparsable file");
    assert_eq!(err.code(), ConfigErrorCode::ConfigParse);

    // `reset` is the sanctioned way out, and a successful write clears the
    // fallbacks it made stale.
...
fn tc_381_126_doctor_reports_valid_invalid_and_key_counts() {
    ...
    match svc.doctor().status {
        DoctorStatus::Invalid { issues } => {
            assert_eq!(issues.len(), 1, "one whole-document issue");
        }
