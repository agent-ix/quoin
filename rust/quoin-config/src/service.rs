...
use crate::paths::{config_path_for, project_config_path_for, Environment, RuntimeContext};
use crate::schema::{KeyKind, PluginConfigSchema};

/// The header every managed config file carries.
const MANAGED_HEADER: &str = "# Managed by quoin — `quoin config edit` to modify safely.\n";
...
impl<'a, S: PluginConfigSchema + Default + serde::Serialize> ConfigService<'a, S> {
    ...
    pub fn set(&self, partial: &Mapping, log: &mut IncidentLog) -> Result<S, ConfigError> {
        let mut next = match read_mapping(&self.user_path) {
            Ok(Some(map)) => map,
            Ok(None) => Mapping::new(),
