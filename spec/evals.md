...
> scenario definitions (TC-EV-001..TC-EV-015) below remain the source of truth; the harness
...
| --- | --- | --- | --- | --- | --- |
| TC-EV-001 | Create an FR plus `domain` and `entity` objects for a small feature | Eval | P1 | US-001 | ✅ |
| TC-EV-002 | Edit an existing FR after discovering its authoring contract | Eval | P1 | US-002 | ✅ |
| TC-EV-003 | Install a local fixture plugin and author one plugin-defined object | Eval | P1 | US-003 | ✅ |
| TC-EV-004 | Validate a mixed changed set of artifact and object files | Eval | P1 | US-004 | ✅ |
| TC-EV-005 | Start a review workflow for a spec directory and inspect status | Eval | P1 | US-005 | ✅ |
| TC-EV-006 | Create multiple artifacts that share object templates | Eval | P1 | US-001 | ✅ |
| TC-EV-007 | Use lowercase type names in an authoring request | Eval | P1 | US-002 | ✅ |
| TC-EV-008 | Repair a spec file after Quire reports validation diagnostics | Eval | P1 | US-004 | ✅ |
| TC-EV-009 | Install, list, remove, and reinstall a local plugin fixture | Eval | P1 | US-003 | ✅ |
| TC-EV-010 | Install a GitHub/package plugin fixture and request one of its types | Eval | P1 | US-003 | ✅ |
| TC-EV-011 | Request an unknown type in an authoring pack | Eval | P1 | US-002 | ✅ |
| TC-EV-012 | Validate multiple globs for a changed-file subset | Eval | P1 | US-004 | ✅ |
| TC-EV-013 | Start matrix and to-plan workflows after accepted requirements | Eval | P1 | US-005 | ✅ |
| TC-EV-014 | Author a complete spec set from a settled conversation | Eval | P1 | US-001 | ✅ |
| TC-EV-015 | Author with sibling development modules present | Eval | P1 | US-001 | ✅ |
| TC-EV-016 | Author into two sibling repos in one session | Eval | P1 | US-001 | ✅ |
| TC-EV-017 | Author a larger feature spec set with cross-references | Eval | P1 | US-001 | ✅ |
| TC-EV-018 | Author objects drawn from three different object modules | Eval | P1 | US-001 | ✅ |
| TC-EV-020 | Install a module from GitHub via a subdir source and author its type | Eval | P1 | US-003 | ✅ |
| TC-EV-021 | Start a new spec from a settled idea (greenfield) | Eval | P1 | US-001 | ✅ |
| TC-EV-022 | Add one user story to an existing spec | Eval | P1 | US-001 | ✅ |
| TC-EV-023 | Edit an existing FR in place | Eval | P1 | US-002 | ✅ |
| TC-EV-024 | Add a user story **and** the FR that implements it | Eval | P1 | US-001 | ✅ |
| TC-EV-025 | Backport a spec from a small source file | Eval | P1 | US-001 | ✅ |
| TC-EV-026 | Run a subset spec-review producing per-analysis SpecReview docs | Eval | P1 | US-005 | ✅ |
| TC-EV-027 | Create an implementation plan as a multi-plan bundle via spec-to-plan | Eval | P1 | US-008 | ✅ |
| TC-EV-028 | Start a second, independent plan in a project that already has one | Eval | P1 | US-008 | ✅ |
| TC-EV-029 | Regenerate an existing plan after a spec change (update in place) | Eval | P1 | US-008 | ✅ |
| TC-EV-030 | Gap-analysis SAD (combined) → FAIL verdict | Eval | P1 | US-005 | ✅ |
| TC-EV-031 | Gap-analysis HAPPY → PASS verdict | Eval | P1 | US-005 | ✅ |
| TC-EV-032 | Gap-analysis SAD (medium-only) → CONDITIONAL verdict | Eval | P1 | US-005 | ✅ |
| TC-EV-033 | Gap-analysis OPTIONAL semantic review catches a hollow test | Eval | P1 | US-005 | ✅ |
| TC-EV-040 | Author an FR whose requirement statements are EARS-clean from the start | Eval | P1 | US-001 | ✅ |
| TC-EV-041 | Repair a seeded FR that trips the EARS requirement-grammar check | Eval | P1 | US-004 | ✅ |
| TC-EV-042 | Author an FR using a project term + define it in a domain's Ubiquitous Language | Eval | P1 | US-001 | ✅ |
| TC-EV-043 | Repair a flagged project term by DEFINING it (not rewording) | Eval | P1 | US-004 | ✅ |
| TC-EV-050 | Generate property tests from settled criteria | Eval | P1 | US-011 | ✅ |
...
| ------ | -------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| TC-EV-001 | US-001         | Create an FR plus `domain` and `entity` objects for a small feature.             | Agent uses one `quoin write` pack, creates all files, and Quire validates them.                                                                                                                                                                                                                                                                                                                                                              | success, latency, tool calls, tokens, validation attempts |
| TC-EV-002 | US-002         | Edit an existing FR after discovering its authoring contract.                    | Agent uses `catalog show` or `write`, edits only needed sections, and validates changed files.                                                                                                                                                                                                                                                                                                                                               | success, latency, tool calls, tokens                      |
| TC-EV-003 | US-003         | Install a local fixture plugin and author one plugin-defined object.             | Plugin appears in catalog, `write --types` resolves it, created file validates.                                                                                                                                                                                                                                                                                                                                                              | success, latency, tool calls, tokens                      |
| TC-EV-004 | US-004         | Validate a mixed changed set of artifact and object files.                       | Agent runs scoped Quire validation over the changed file set and reports failures clearly.                                                                                                                                                                                                                                                                                                                                                   | success, latency, tool calls, validation attempts         |
| TC-EV-005 | US-005         | Start a review workflow for a spec directory and inspect status.                 | `quoin` starts the workflow and `ix-flow status` reports the run and gate state.                                                                                                                                                                                                                                                                                                                                                             | success, latency, tool calls                              |
| TC-EV-006 | US-001, US-004 | Create multiple artifacts that share object templates.                           | Agent fetches each type contract once, reuses it across files, and validates all changed files.                                                                                                                                                                                                                                                                                                                                              | success, context fetches, tokens, tool calls              |
| TC-EV-007 | US-002         | Use lowercase type names in an authoring request.                                | `quoin write . --types fr,domain` resolves canonical types and prints usable contracts.                                                                                                                                                                                                                                                                                                                                                      | success, latency, tool calls                              |
| TC-EV-008 | US-004         | Repair a spec file after Quire reports validation diagnostics.                   | Agent interprets diagnostics, edits the failing file, and reaches passing validation.                                                                                                                                                                                                                                                                                                                                                        | success, latency, tool calls, tokens, validation attempts |
| TC-EV-009 | US-003         | Install, list, remove, and reinstall a local plugin fixture.                     | Plugin registry changes are isolated to `IX_HOME`; catalog reflects each lifecycle step.                                                                                                                                                                                                                                                                                                                                                     | success, latency, tool calls                              |
| TC-EV-010 | US-003         | Install a GitHub/package plugin fixture and request one of its types.            | `write --types` resolves the installed type and prints usable paths.                                                                                                                                                                                                                                                                                                                                                                         | success, latency, tool calls, tokens                      |
| TC-EV-011 | US-002         | Request an unknown type in an authoring pack.                                    | CLI reports the missing type clearly and exits non-zero.                                                                                                                                                                                                                                                                                                                                                                                     | success, latency, tool calls                              |
| TC-EV-012 | US-004         | Validate multiple globs for a changed-file subset.                               | Quire validates only matching files and reports each invalid file with a path.                                                                                                                                                                                                                                                                                                                                                               | success, latency, tool calls, validation attempts         |
| TC-EV-013 | US-005         | Start matrix and to-plan workflows after accepted requirements.                  | `quoin matrix` and `quoin to-plan` create runs that `ix-flow status` can inspect.                                                                                                                                                                                                                                                                                                                                                            | success, latency, tool calls                              |
| TC-EV-014 | US-001, US-002 | Author a complete spec set from a settled conversation.                          | Agent creates spec, use cases, matrix, plan, and eval matrix, then validates all spec files.                                                                                                                                                                                                                                                                                                                                                 | success, latency, tool calls, tokens, context fetches     |
| TC-EV-015 | US-001, US-004 | Author with sibling development modules present.                                 | Catalog prefers intended dev modules deterministically and validation uses matching contracts.                                                                                                                                                                                                                                                                                                                                               | success, latency, tool calls, tokens                      |
| TC-EV-016 | US-001, US-004 | Author into two sibling repos in one session.                                    | Agent authors under `core/spec/` and `service/spec/` and validates both with one scoped run.                                                                                                                                                                                                                                                                                                                                                 | success, latency, tool calls, tokens                      |
| TC-EV-017 | US-001, US-002 | Author a larger feature spec set with cross-references.                          | Agent authors StR + 2 US + 3 FR + NFR + domain + 2 entity, fetching each contract once; all validate.                                                                                                                                                                                                                                                                                                                                        | success, latency, tool calls, tokens, context fetches     |
| TC-EV-018 | US-001         | Author objects drawn from three different object modules.                        | One `write` pack resolves `domain`/`api_endpoint`/`configuration` across modules; all validate.                                                                                                                                                                                                                                                                                                                                              | success, latency, tool calls, tokens                      |
| TC-EV-020 | US-003         | Install a module from GitHub via a subdir source and author its type.            | Agent runs `quoin plugin install github:owner/repo//subdir@ref` (real network clone), then authors and validates one of its types.                                                                                                                                                                                                                                                                                                           | success, latency, tool calls, tokens                      |
| TC-EV-021 | US-001         | Start a new spec from a settled idea (greenfield).                               | Agent creates `spec.md` plus ≥1 US and ≥1 FR **as discrete files**; all validate. (`artifacts` check)                                                                                                                                                                                                                                                                                                                                        | success, latency, tool calls, tokens                      |
| TC-EV-022 | US-001         | Add one user story to an existing spec.                                          | Agent adds a new `US` file and creates **no** FR/NFR/IT artifacts; all validate. (`artifacts` require + absent)                                                                                                                                                                                                                                                                                                                              | success, latency, tool calls                              |
| TC-EV-023 | US-002         | Edit an existing FR in place.                                                    | Agent modifies the FR file only, creating no new artifact types; the FR validates. (`artifacts` require + absent)                                                                                                                                                                                                                                                                                                                            | success, latency, tool calls, validation attempts         |
| TC-EV-024 | US-001         | Add a user story **and** the FR that implements it.                              | Agent authors **both** a `US` and an `FR` artifact (FR traced to the US); all validate. (`artifacts` require)                                                                                                                                                                                                                                                                                                                                | success, latency, tool calls, tokens                      |
| TC-EV-025 | US-001         | Backport a spec from a small source file.                                        | Agent authors ≥1 `FR` artifact under `spec/functional/` (not just a `spec.md` table) capturing the code's behavior; all validate.                                                                                                                                                                                                                                                                                                            | success, latency, tool calls, tokens                      |
| TC-EV-026 | US-005         | Run a subset spec-review producing per-analysis SpecReview docs.                 | Agent runs a `subset` spec-review (integrity + dependency) and authors one **validated** `SpecReview` doc per selected analysis under `spec/reviews/` (Summary + Findings table, Severity enum). (`files` + `validate`)                                                                                                                                                                                                                      | success, latency, tool calls, tokens, validation attempts |
...
| TC-EV-058 | US-001         | Author an explicitly requested installed assurance profile.                       | With the local engineering-assurance module installed, the agent fetches the live `AssuranceProfile` contract once, authors exactly one linked and validated profile, and creates no architecture, measurement, or review artifact. Run on both Claude and Codex. (`agentRan` + `artifacts` + `fileContains` + `validate`) | success, latency, tool calls, tokens, validation attempts |
| TC-EV-059 | US-001         | Preserve ordinary `/specify` behavior when assurance is installed but unrequested. | With the same module installed, an ordinary contained FR request produces exactly one validated FR and no assurance, architecture, measurement, or review artifact. Run on both Claude and Codex. (`artifacts` + `validate`) | success, latency, tool calls, tokens, validation attempts |
...
> are harvested and accepted as concrete in its grammar check. TC-EV-042 authors
> with the definition present, TC-EV-043 repairs a flagged project term by defining
...
> TC-EV-050..TC-EV-053 exercise the **`spec-correctness`** skill
...
> tests. TC-EV-050 covers the unattended lane, TC-EV-051 the review-gated lane and the
> byte-identical tag through acceptance, TC-EV-052 the handoff to `gap-analysis`,
> TC-EV-053 the refusals and the CON-1 no-verdict rule. All four are implemented in
...
> for TC-EV-050..TC-EV-052, and for TC-EV-053 two criteria the classifier calls extractable
...
> TC-EV-016..TC-EV-018 and TC-EV-020 extend the original TC-EV-001..TC-EV-015 set (multi-repo,
> larger spec-set, multi-module, real GitHub subdir install). TC-EV-021..TC-EV-025 are
...
`pnpm run test:evals` builds the CLI, runs TC-EV-001 through TC-EV-015 in disposable
