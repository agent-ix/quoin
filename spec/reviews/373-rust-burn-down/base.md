...
| FND-004 | medium | NFR-024 requires every retained path to name a valid successor reference, defined as an open sub-issue of the burn-down epic naming its delivery stage and the path it retires. When this set was authored no such issue existed and EPIC #373 carried the stages in prose only; the delivery-stage issues were created on 2026-09-12 and the matrix linked to them at a353681. NFR-024 keeps a **provisional** class for an epic-level reference so a regression to one is visible. | NFR-024, EPIC #373, `docs/rust-burndown/executable-path-matrix.md` | missing-requirement |
...
| The successor rule refuses the matrix that exists (SR-155 FND-004) | Resolved. The matrix now names real stage issues; an epic-level reference is reported as **provisional**, distinct from valid and invalid, satisfying nothing (NFR-024-AC-10). |
| Delivery-stage tickets 0–9 do not exist (SR-153 FND-003, SR-154 FND-013, SR-156 FND-183) | Resolved outside the spec. The stage issues were created on 2026-09-12 and the matrix links them at a353681; NFR-024 keeps a provisional class so an epic-level reference cannot return unnoticed. |
...
| Delivery-stage tickets, allowance manifest and matrix must be created (SR-153 FND-003, SR-155 FND-004/005) | Resolved outside the spec: `.language-allowances.yaml` landed at 2812ecb and the stage issues at a353681. |
