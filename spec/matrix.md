...
| FR-072 | ✅ Covered | `tests/semantic-mapping.test.ts` — TC-1353..TC-1359, TC-1715: `ClauseRef`/`OperationDecl` extraction, `quire` checked and `ocl` carried under semantic-core `0.2.0`, language and duplicate rejection, dangling pre/post, inline-vs-external duplicate. |
...
| TC-1715 | Under semantic-core `0.2.0`, the `fence-ocl-carried` case carries its `ocl` body verbatim with one `semantic.clause-language-unchecked` advisory at the fence and a lossy clauses kind | Unit | P0 | FR-072-AC-7 | ✅ Complete |
