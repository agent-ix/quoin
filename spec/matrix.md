Warning: truncated output (original token count: 30396)
...
| TC-263 | `evidence gc` through the command: dry-run JSON lists without deleting, a real run deletes and reports each path, an absent store collects nothing — and the flag set is exactly `repo`/`dry-run`/`json`, pinning the deliberate absence of `--module` (#171: gc never…396 tokens truncated…d `lower-is-better` — the rate cannot fall independently of the corpus's own differential gate, so the count is what carries the evidence (#245) | Unit | P0 | FR-043-AC-16 | ✅ |
