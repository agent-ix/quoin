...
| FR-043 | ✅ Covered | `tests/measurement-record.test.ts` — TC-277..TC-282. The latency-distribution and per-function-complexity fixtures prove the schema is measure-neutral; invalid identity and every numeric position are refused before write; record identity and canonical bytes are deterministic; provenance round-trips; and the closed schema admits no policy, named standard, or hard-coded measure. CON-2/CON-3 → inspection of `src/evidence/` (no tool execution and no RunEntry conversion). |
| FR-044 | ✅ Covered | `tests/measurement-comparison.test.ts` — TC-283..TC-289. Exact queries, two unlike fixture families through one comparator, identity discontinuities, five absent/partial/ambiguous states, explicit ratchet application, ordered trends, deterministic JSON/Markdown, and engine neutrality are covered. CON-3 → inspection of `src/evidence/comparison.ts` (no FindingRecord construction). |
| FR-045 | ✅ Covered | `tests/measurement-adapters.test.ts` — TC-290..TC-296. Selected structural/churn/clone adapters, separate dimensions, stable subject identity, limitation/duplicate/outside-path refusal, non-vacuous dependency-rule scans, atomic command intake, and the normalized extension seam are covered. CON-1/CON-3 → inspection of `src/evidence/adapters/measurement-observations.ts` and `src/commands/evidence/measure.ts` (pure parsing; no subprocess or producer execution). |
