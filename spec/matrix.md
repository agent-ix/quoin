...
- `core-modules.test.ts` — the boundary caller for install/list/remove and the
  registry key order it restores; the rules themselves moved to `quoin-modules`
  in quoin#446, where `ts_oracle.rs` and `install_reconcile.rs` carry them.
- `core-org.test.ts` / `org-one-subprocess.test.ts` — the boundary caller for
  org resolution, the project-config path it derives, and the one subprocess it
  is allowed to execute; the rules moved to `quoin-config`, where
  `ts_oracle.rs`, `config_service.rs` and the `tc_446_*` files carry them.
...
| FR-018      | ✅ Covered | `quoin-modules` `ts_oracle.rs` :: `tc_381_230` (every `path:`/`github:`/`github://subdir`/`package:`/bare form, against the captured TypeScript answers); `tc_446_source_arg_properties.rs` :: `tc_446_060`…`tc_446_065` (the same five forms as exhaustive loops over a bounded domain, with a population floor); `index.test.ts` :: library install from a bare path |
| FR-019      | ✅ Covered | `quoin-modules` `install_reconcile.rs` :: `tc_381_280` (install, list, then remove a path module, target dir and registry entry both); `ts_oracle.rs` :: `tc_381_237` (module name from a top-level and a single-nested `manifest.yaml`, and the error code when neither is there); `core-modules.test.ts` :: the boundary caller and the registry key order it prints; `cli.test.ts` :: "install without a source throws"; :: "remove without a name throws"; :: "remove deletes a module and prints confirmation" |
...
| FR-023 | ✅ Covered | `cli.test.ts` :: "falls back to IX_HOME when --config-root is omitted, and prints 'unknown' for a versionless module"; `catalog.test.ts` :: "includes QUOIN_MODULE_PATHS entries and installed module dirs"; `flows.test.ts` :: "resolves when ix-flow exits 0; builds id/json/target args" (IX_SPEC_WORKFLOWS_ROOT); AC-4 (`--org`/`QUOIN_ORG`, neither defaulting) via `quoin-config` `ts_oracle.rs` :: `tc_381_104` (flag and env precedence, against the captured answers) and `tc_446_org_parity.rs` :: `tc_446_026` (flag over env over file over remote, on both the filesystem and the document path); `cli.test.ts` :: "--org reaches the pack in the text rendering"; :: "reports an unresolved org with the --org remedy" |
...
| FR-025 | ✅ Covered | Resolution moved into `quoin-config` at quoin#446, so the criteria are carried there and the boundary caller is pinned from this side. AC-1 `tc_446_org_parity.rs` :: `tc_446_026` (flag over env over user file over project file over remote, and a blank flag falling through, asserted on the filesystem path and the document path and required to agree); `ts_oracle.rs` :: `tc_381_104`; `core-org.test.ts` :: each of the three documents reached as the only source of an org, over a real tree, with a population floor. AC-2/AC-3 `ts_oracle.rs` :: `tc_381_100` (every captured SSH and HTTPS url); `tc_446_org_url_properties.rs` :: `tc_446_052` (both forms across their whole host × owner × suffix matrix). AC-4 `ts_oracle.rs` :: `tc_381_101`, `tc_381_107`; `core-org.test.ts` :: the unresolved-org sentence served over the boundary. AC-5 `quoin-config` `ids.rs` :: `OrgName` refuses to substitute; `write.test.ts` :: "reports unresolved with the remedy and no substituted value". AC-6 `write.test.ts` :: "carries an explicit --org value and its source"; :: "carries QUOIN_ORG when no flag is given"; :: "serializes the org into the --json rendering"; `cli.test.ts` :: "--org reaches the pack in the text rendering"; :: "--org reaches the pack under --json"; :: "reports an unresolved org with the --org remedy". AC-7 (amended at quoin#446: one subprocess, and it is `quoin-core`) `tc_446_org_no_child_program.rs` :: `tc_446_070` (the resolver names no child-program capability) and `tc_446_071` (the control that proves the audit fails when one is named); `org-one-subprocess.test.ts` :: every `node:child_process` entry point recorded, exactly one call, `quoin-core`, never `git`. AC-8 `ts_oracle.rs` :: `tc_381_105`, `tc_381_106`; `core-org.test.ts` :: "follows a worktree's .git file to the common directory". AC-9 `ts_oracle.rs` :: `tc_381_100`; `tc_446_org_url_properties.rs` :: `tc_446_050`. AC-10 `tc_446_org_url_properties.rs` :: `tc_446_051`. AC-11 `ts_oracle.rs` :: `tc_381_101`; `tc_446_org_url_properties.rs` :: `tc_446_053` |
...
| FR-027 | ✅ Covered | AC-1…AC-5 and AC-9 moved into `quoin-config` at quoin#446. AC-1 `ts_oracle.rs` :: `tc_381_103`; `tc_446_org_parity.rs` :: `tc_446_026`; `config_service.rs` :: the layered read. AC-2 `tc_446_org_parity.rs` :: `tc_446_026` (an explicit flag over a stored value). AC-3 same test (`QUOIN_ORG` layered over the file and reported as `env`); `ts_oracle.rs` :: `tc_381_103`. AC-4 `tc_446_org_parity.rs` :: `tc_446_026` (fall-through to the remote, then to unresolved). AC-5 `tc_446_org_parity.rs` :: `tc_446_027` (a broken layer degrades identically on both paths and stops neither), `ts_oracle.rs` :: `tc_381_103`, `config_service.rs`. AC-6 `config-schema.test.ts` :: "rejects an unrecognized key"; :: "rejects an empty org"; :: "accepts a non-empty org"; :: "accepts an absent org rather than defaulting one". AC-7 :: "declares the plugin id, schema, and env binding". AC-8 `cli.test.ts` :: "set stores an org that write then resolves"; :: "get resolves for a stored key without erroring"; :: "set rejects an unrecognized key"; :: "doctor reports on a clean config without failing"; :: "edit opens the config file through the shared handler"; `config-schema.test.ts` :: "get calls runConfigGet from the shared package"; :: "set calls runConfigSet from the shared package"; :: "edit calls runConfigEdit from the shared package"; :: "doctor calls runConfigDoctor from the shared package"; :: "each command registers quoin's schema before delegating". AC-9 `tc_446_org_parity.rs` :: `tc_446_026` (the project layer beating the user layer); `config_service.rs` :: the project layer ignored where the invocation disables it; `core-org.test.ts` :: "reads the project layer at the derived path"; :: "ignores the project layer when the invocation disables it"; :: "is the path ix-cli-core's own ConfigService reads" |
...
| FR-044 | ✅ Covered | `src/measurement/` was deleted at [quoin#479](https://github.com/agent-ix/quoin/issues/479); the contract is now `quoin-measurement`, reached through `quoin-core`. `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` — TC-1003 (unplanned records refused), TC-1004 (atomic/idempotent collection), TC-1005 (definition/config refusal names both), TC-1006 (population movement beside delta; no severity), TC-1007 (missing metric is `not_computed`), TC-1008 (byte-identical report retains plans without records). Tier-1 envelope provenance/raw attachment and explicit legacy history remain TC-997/TC-998/TC-1000. |
| FR-063 | ✅ Covered | `src/change-assurance/` was deleted at [quoin#457](https://github.com/agent-ix/quoin/issues/457); the contract is now `quoin-change-assurance`, reached through `quoin-core change_assurance.*`. `quoin-change-assurance` `tc_455_records.rs`, `tc_455_ix_flow.rs` and `tc_455_boundary.rs` TC-1261..TC-1271 verify the closed record contract, complete collections, proof premises, visible unknowns, RFC 8785/BLAKE3 sealing, mutation and malformed-input refusal, immutable lineage, exact ix-flow decisions, and non-identity boundary. |
| FR-064 | ✅ Covered | `src/change-assurance/` was deleted at [quoin#457](https://github.com/agent-ix/quoin/issues/457); the contract is now `quoin-change-assurance`, reached through `quoin-core change_assurance.*`. `quoin-change-assurance` `tc_455_attestations.rs`, `tc_455_intake.rs` and `tc_455_boundary.rs` TC-1272..TC-1280 verify closed attestation bindings/states, exact output integrity, canonical sealing, missing-field refusal, crash-atomic/idempotent/collision-safe intake, unavailable diagnostics, FR-030 compatibility, and no-execution/non-verdict boundaries. |
| FR-065 | ✅ Covered | `src/change-assurance/` was deleted at [quoin#457](https://github.com/agent-ix/quoin/issues/457); the contract is now `quoin-change-assurance`, reached through `quoin-core change_assurance.*`. `quoin-change-assurance` `tc_455_verify.rs` and `tc_455_boundary.rs` TC-1281..TC-1292 verify receipt shape and precedence, explicit selections, exact binding, all unhealthy/missing evidence states, ix-flow decisions, incomplete premises, unchanged FR-032 findings, determinism, compatibility, and non-identity terminology. |
| FR-045 | ✅ Covered | `src/measurement/portfolio.ts` was deleted at [quoin#479](https://github.com/agent-ix/quoin/issues/479); the contract is now `quoin-measurement`, with the surviving `quoin report` command surface on this side of the boundary in `tests/report-portfolio-command-surface.test.ts`. `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` — TC-1011 (mixed definitions, missing/unreadable stores, relative staleness), TC-1012 (plan/collection links, same-object deterministic human/JSON views, no aggregate), TC-1013 (one command accepts repeated repository paths and no values). |
...
| FR-056 | ✅ Covered | `src/measurement/intervention*.ts` was deleted at [quoin#479](https://github.com/agent-ix/quoin/issues/479); the contract is now `quoin-measurement`. `tc_479_fr056_criteria.rs` — TC-1195..TC-1203 cover the versioned record envelope, unchanged producer tuple, design/arm/effect boundaries, qualifiers, causal-safety rules, governance, and exact raw evidence. |
| FR-057 | ✅ Covered | `src/measurement/intervention.ts` was deleted at [quoin#479](https://github.com/agent-ix/quoin/issues/479); the contract is now `quoin-measurement`. `tc_479_fr057_criteria.rs` — TC-1204..TC-1216 cover atomic governed intake, stable refusals, idempotence/collision handling, negative outcomes, deterministic claim-centered rendering, no execution path, and legacy-store compatibility. |
| FR-058 | ✅ Covered | `src/measurement/agent-eval-intervention.ts` was deleted at [quoin#479](https://github.com/agent-ix/quoin/issues/479); the contract is now `quoin-measurement`. `tc_479_fr058_criteria.rs` — TC-1217..TC-1221 consume the two byte-exact real Codex reports under `spec/evidence/agent-evals/`, derive the observed 0.5 → 0.5 effect, retain both SHA-256 digests, refuse invalid report families, force `cause_not_established`, and prove the adapter does not execute the experiment. |
| FR-059 | ✅ Covered | `src/measurement/operational*.ts` was deleted at [quoin#479](https://github.com/agent-ix/quoin/issues/479); the contract is now `quoin-measurement`. `tc_479_fr059_criteria.rs` — TC-1223..TC-1231 cover the versioned operational envelope, full control vocabulary, standing/exercise distinction, clock derivation, typed pins, adverse outcomes, governance, and exact raw evidence. |
| FR-060 | ✅ Covered | `src/measurement/operational.ts` was deleted at [quoin#479](https://github.com/agent-ix/quoin/issues/479); the contract is now `quoin-measurement`. `tc_479_fr060_criteria.rs` — TC-1232..TC-1243 cover atomic governed intake, stable refusals, idempotence/collision handling, clocked discharge, deterministic claim-centered reporting, no control-execution path, and legacy-store compatibility. |
| FR-061 | ✅ Covered | `src/measurement/github-release-operational.ts` was deleted at [quoin#479](https://github.com/agent-ix/quoin/issues/479); the contract is now `quoin-measurement`. `tc_479_fr061_criteria.rs` — TC-1244..TC-1248 consume the byte-exact v0.22.5 workflow/run/jobs artifacts under `spec/evidence/github-actions/`, derive and atomically persist the linked operational pair, refuse malformed or mismatched inputs, preserve adverse outcomes, and prove the adapter remains offline. |
| FR-062 | ✅ Covered | TC-1249..TC-1260 verify distinct suite fan-out, unresolved bindings, deterministic reverse dependency closure, unchanged auditor verdicts, deduplicated reaffirmation events, availability states, source premises, rendering determinism, read-only boundaries, and non-scoring in the `quoin-graph-analysis` `tc_385_*` and `tc_500_restated_criteria.rs` tests, the `quoin-core` `ops/graph` tests, and `graph-command.test.ts`. |
| FR-066 | ✅ Covered | `src/measurement/graph-adapters.ts` was deleted at [quoin#480](https://github.com/agent-ix/quoin/issues/480); the contract is now `quoin-measurement-graph`, reached through `quoin-core measurement.*`. `quoin-measurement-graph` `tc_475_parity.rs` and `tc_480_fr066_criteria.rs` — TC-1293..TC-1304 cover the exact adapter registry, fail-closed Quire intake, lossless graph handoff, closed graph-quality schema and identity, exact raw attachment, invocation attestation, plan gating, bijective normalization, non-measured states, atomic intake, and no-execution boundaries. |
| FR-067 | ✅ Covered | `src/measurement/graph-portfolio.ts` and `src/measurement/graph-portfolio-load.ts` were deleted at [quoin#480](https://github.com/agent-ix/quoin/issues/480); the contract is now `quoin-measurement-graph`, reached through `quoin-core measurement.build_graph_portfolio` and `measurement.render_graph_portfolio`. `quoin-measurement-graph` `tc_476_graph_portfolio.rs`, `tc_477_graph_loader.rs` and `tc_480_fr067_criteria.rs`, with `tests/graph-portfolio-command.test.ts` on this side of the boundary — TC-1305..TC-1315 cover current/history evidence, partitions and provenance, distinct availability, compatibility-gated comparison, raw identities, exact FR-062 embedding, local failures, determinism, backward compatibility, and read-only/non-scoring boundaries. |
| FR-069 | ✅ Covered | `tests/campaign-adapters.test.ts` — TC-1328..TC-1335 cover the two campaign-native formats against real pinned samples, the unsupported state that no run-entry outcome carries, fail-closed refusals, registry selection, the record command's reporting of unrepresented results, the published inventory, and the no-execution and no-scraping boundaries. `tests/contract-conformance-traces.test.ts` — TC-1585..TC-1588 cover trace preservation, legacy compatibility, malformed metadata, criterion binding through Quire, and no partial evidence. |
...
| FR-072 | ⚠️ Delegated | quoin publishes the goldens under `tests/fixtures/semantic-module/mapping/`; quoin executes none of them. agent-ix/quire-rs `tests/semantic_clauses.rs` executes them — TC-1353/TC-1356 `golden_operations_and_spans`, TC-1354 `language_cases`, TC-1355/TC-1358 `structural_cases`, TC-1357 `operation_cases`, TC-1715 `quire_is_checked_and_ocl_is_carried`; TC-1716 awaits agent-ix/quire-rs#432. CON-1 → quoin `tc_452_sweep_criteria.rs` :: `tc_452_653` (TC-1359). |
...
| FR-068 | ✅ Covered | The FR-063..FR-065 contracts moved into `quoin-change-assurance` at [quoin#457](https://github.com/agent-ix/quoin/issues/457), so the criteria about what the commands DECIDE are carried against the real `quoin-core` binary and the criteria about what stays this side of the boundary are carried in TypeScript. `quoin-core` `tc_457_change_assurance_boundary.rs` — TC-1317..TC-1321, TC-1323, TC-1325 cover record and attestation sealing from explicit inputs, atomic exact-byte intake and idempotence, receipt assembly from named selections only, preserved unavailable/not-computed/missing states, receipt re-verification, and staging recovery; every operation is reached by its wire spelling through the binary. `tests/change-assurance-command-surface.test.ts` — TC-1322, TC-1324, TC-1326, TC-1327 cover the 0/1/2 exit grammar, packaged schema emission, the canonical goldens a consumer already stored, and the boundaries (amended CON-1: the one process any command starts is the FR-096 engine). |
| StR-007 | ✅ Covered | `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_048` TC-1316 retains producer identity, partitions, availability, and comparison premises through the adapter-to-portfolio flow without execution or an aggregate verdict. |
| StR-009 | ⚠️ Spec-ahead-of-code | TC-1601..TC-1604 — one implementation language for engine logic and one schema source. 5 criteria, none implemented: the port has not started. Spec-ahead-of-code by design (#373). |
...
| FR-104 | ⚠️ Delegated | quoin publishes the goldens `relationships.md`, `relationships.expected.json`, and `relationships-cases.json` under `tests/fixtures/semantic-module/mapping/`; quoin executes none of them. TC-1717..TC-1726 await agent-ix/quire-rs#418, which executes them. |
...
| NFR-003     | ✅ Covered | `write.test.ts` :: "throws with available type list when a type is not found"; `cli.test.ts` :: "an unknown command is rejected by the runner"; `quoin-modules` `ts_oracle.rs` :: `tc_381_237` (the error code when no `manifest.yaml` exists)                                                                                   |
...
| NFR-024 | ⚠️ Spec-ahead-of-code | TC-1660..TC-1664, TC-1686..TC-1689, TC-1705..TC-1708, TC-1710 — bounded staged coexistence — every retained path carries a successor and an expiry, the allowance manifest is checked in, and the enforcement run reports an empty population as inconclusive. 14 criteria, none implemented: the port has not started. Spec-ahead-of-code by design (#373). |
...
| FR-069 | FR-069-AC-1, FR-069-AC-2, FR-069-AC-3, FR-069-AC-4, FR-069-AC-5, FR-069-AC-6, FR-069-AC-7, FR-069-AC-8, FR-069-AC-9, FR-069-AC-10, FR-069-AC-11 | TC-1328, TC-1329, TC-1330, TC-1331, TC-1332, TC-1333, TC-1334, TC-1335, TC-1585, TC-1586, TC-1587, TC-1588 | ✅ Covered |
...
| FR-072 | FR-072-AC-1, FR-072-AC-2, FR-072-AC-3, FR-072-AC-4, FR-072-AC-5, FR-072-AC-6, FR-072-AC-7, FR-072-AC-8, FR-072-CON-1 | TC-1353, TC-1354, TC-1355, TC-1356, TC-1357, TC-1358, TC-1359, TC-1715, TC-1716 | ⚠️ Partial |
...
| TC-060 | FR-018-AC-5 covered by `quoin-modules` `tc_446_source_arg_properties.rs` :: `tc_446_060` | Property | P1 | FR-018-AC-5 | ✅ |
| TC-061 | FR-019-AC-1 covered by `cli.test.ts` | Unit | P1 | FR-019-AC-1 | ✅ |
| TC-062 | FR-019-AC-2 covered by `quoin-modules` `ts_oracle.rs` :: `tc_381_237` | Unit | P1 | FR-019-AC-2 | ✅ |
...
| TC-078 | FR-023-AC-4 covered by `cli.test.ts`, `quoin-config` `ts_oracle.rs` :: `tc_381_104` | Unit | P1 | FR-023-AC-4 | ✅ |
...
| TC-083 | FR-025-AC-1 covered by `quoin-config` `tc_446_org_parity.rs` :: `tc_446_026`, `core-org.test.ts` | Unit | P1 | FR-025-AC-1 | ✅ |
| TC-084 | FR-025-AC-2 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_100`, `tc_446_org_url_properties.rs` :: `tc_446_052` | Property | P1 | FR-025-AC-2 | ✅ |
| TC-085 | FR-025-AC-3 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_100`, `tc_446_org_url_properties.rs` :: `tc_446_052` | Property | P1 | FR-025-AC-3 | ✅ |
| TC-086 | FR-025-AC-4 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_101`, `tc_381_107` | Unit | P1 | FR-025-AC-4 | ✅ |
| TC-087 | FR-025-AC-5 covered by `quoin-config` `ids.rs`, `write.test.ts` | Unit | P1 | FR-025-AC-5 | ✅ |
| TC-088 | FR-025-AC-6 covered by `cli.test.ts`, `write.test.ts` | Unit | P1 | FR-025-AC-6 | ✅ |
| TC-089 | FR-025-AC-7 (amended at quoin#446) covered by `quoin-config` `tc_446_org_no_child_program.rs` :: `tc_446_070`, `tc_446_071`; `org-one-subprocess.test.ts` | Unit | P1 | FR-025-AC-7 | ✅ |
| TC-090 | FR-025-AC-8 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_105`, `tc_381_106`; `core-org.test.ts` | Unit | P1 | FR-025-AC-8 | ✅ |
| TC-091 | FR-025-AC-9 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_100`, `tc_446_org_url_properties.rs` :: `tc_446_050` | Property | P1 | FR-025-AC-9 | ✅ |
| TC-092 | FR-025-AC-10 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_100`, `tc_446_org_url_properties.rs` :: `tc_446_051` | Property | P1 | FR-025-AC-10 | ✅ |
| TC-093 | FR-025-AC-11 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_101`, `tc_446_org_url_properties.rs` :: `tc_446_053` | Property | P1 | FR-025-AC-11 | ✅ |
...
| TC-101 | FR-027-AC-1 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_103`, `tc_446_org_parity.rs` :: `tc_446_026` | Unit | P1 | FR-027-AC-1 | ✅ |
| TC-102 | FR-027-AC-2 covered by `quoin-config` `tc_446_org_parity.rs` :: `tc_446_026` | Unit | P1 | FR-027-AC-2 | ✅ |
| TC-103 | FR-027-AC-3 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_103`, `tc_446_org_parity.rs` :: `tc_446_026` | Unit | P1 | FR-027-AC-3 | ✅ |
| TC-104 | FR-027-AC-4 covered by `quoin-config` `ts_oracle.rs` :: `tc_381_103`, `tc_446_org_parity.rs` :: `tc_446_026` | Unit | P1 | FR-027-AC-4 | ✅ |
| TC-105 | FR-027-AC-5 covered by `quoin-config` `tc_446_org_parity.rs` :: `tc_446_027`, `config_service.rs` | Unit | P1 | FR-027-AC-5 | ✅ |
...
| TC-109 | FR-027-AC-9 covered by `quoin-config` `config_service.rs`, `tc_446_org_parity.rs` :: `tc_446_026`; `core-org.test.ts` | Unit | P1 | FR-027-AC-9 | ✅ |
...
| TC-1003 | A metric with no authored active MeasurementPlan is refused by name rather than stored untyped | Unit | P0 | FR-044-AC-1 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_001`, `tc_479_002`, `tc_479_003`, `tc_479_004` |
| TC-1004 | One complete producer collection is written atomically and an identical id/content write is byte-idempotent | Unit | P0 | FR-044-AC-2 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_005` |
| TC-1005 | Changed definition and configuration refuse a delta and name both incompatible fields | Unit | P0 | FR-044-AC-3 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_006` |
| TC-1006 | A population change stays beside a numeric delta and the comparison carries no verdict or severity | Unit | P0 | FR-044-AC-3 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_007` |
| TC-1007 | A metric present in only one collection is `not_computed`, never zero or unchanged | Unit | P0 | FR-044-AC-3 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_008` |
| TC-1008 | Re-rendering unchanged plans/store is byte-identical and an authored plan with no record remains visible as `not_computed` | Unit | P0 | FR-044-AC-4 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_009`, `tc_479_010`, `tc_479_011` |
...
| TC-1011 | A five-repository fixture preserves a changed definition as incomparable, names a missing store and unreadable collection, and marks exactly the repository more than 30 days behind the newest collection stale (#248) | Unit | P0 | FR-045-AC-2, FR-045-AC-3 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_013`, `tc_479_014` |
| TC-1012 | Human and canonical JSON portfolio views derive from one report; every observation links its plan and collection; no cross-repository aggregate is emitted (#248) | Unit | P0 | FR-045-AC-3, FR-045-AC-4 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_015`, `tc_479_016` |
| TC-1013 | One `quoin report` invocation accepts repeated `--portfolio` repository locations and exposes no typed observation-value flag (#248) | Integration | P0 | FR-045-AC-1 | ✅ `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_012`, `tests/report-portfolio-command-surface.test.ts` |
...
| TC-1071 | The shipped `quoin validate --json` command emits the same located finding as the validator and remains advisory unless `--strict` is selected (#224). Carried since [#412](https://github.com/agent-ix/quoin/issues/412) by `tests/core-exec-e2e.test.ts`, which drives the real oclif command against the real `quoin-core` binary. It is deliberately **not** carried by a Rust test as FR-101-AC-4 would prefer: the criterion is about the shipped oclif command, which is TypeScript until FR-102 retires the command surface, and `golden_parity.rs::tc_377_019` — which used to name TC-1071 — calls the library in process and never invokes the command ([#448](https://github.com/agent-ix/quoin/pull/448) FND-004) | Integration | P0 | FR-043-AC-20 | ✅ |
...
| TC-1195 | The v1 schema requires record and subject identities, an observation timestamp, and every unchanged FR-044 producer-tuple field | Unit | P0 | FR-056-AC-1 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_100` |
| TC-1196 | Each missing producer field, mutable tool version, empty environment, and malformed configuration digest is rejected independently | Unit | P0 | FR-056-AC-2 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_101` |
| TC-1197 | Repeated, randomized, and factorial designs accept their valid assignment combinations; randomized assignment without a seed, zero repetitions, and empty sampling conditions fail at their boundaries | Unit | P0 | FR-056-AC-3 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_102` |
| TC-1198 | Unique baseline and treatment ids, treatment-linked changed variables, and an explicit held-constant list validate; duplicate or dangling treatment references fail | Property | P0 | FR-056-AC-4 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_103` |
| TC-1199 | Treatment-linked numeric, textual, boolean, and unavailable effect values round-trip without coercing unavailable to zero; duplicate treatment/metric keys fail | Property | P0 | FR-056-AC-5 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_104` |
| TC-1200 | Interaction and confounder entries preserve all four dispositions and never move between the two collections | Property | P0 | FR-056-AC-6 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_105` |
| TC-1201 | Completed, failed, and inconclusive records validate, while failed/inconclusive records with any conclusion other than `cause_not_established` fail | Unit | P0 | FR-056-AC-7 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_106` |
| TC-1202 | Causal and no-effect conclusions require positive samples and observed comparisons; causal conclusions also require non-null effects, non-none confidence, and no uncontrolled/unknown qualifier | Unit | P0 | FR-056-AC-8 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_107` |
| TC-1203 | Gaps, owner, actions, and safe content-digested raw evidence with media type and byte size are required, and every undeclared field is refused recursively | Property | P0 | FR-056-AC-9 | ✅ `quoin-measurement` `tc_479_fr056_criteria.rs` :: `tc_479_108` |
| TC-1204 | Valid intake writes exactly one complete canonical record by one atomic same-directory no-replace publication, and an existing concurrent destination is never overwritten | Integration | P0 | FR-057-AC-1 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_200` |
| TC-1205 | Every invalid schema or cross-record integrity path is reported with `invalid_record` and no store entry is written | Unit | P0 | FR-057-AC-2 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_201` |
| TC-1206 | An absent or mismatched governing definition returns its stable reason code, names requested, expected, and observed versions, and leaves the store unchanged | Unit | P0 | FR-057-AC-3 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_202` |
| TC-1207 | Repeating identical canonical bytes for one id is byte-idempotent | Property | P0 | FR-057-AC-4 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_203` |
| TC-1208 | A same-id/different-bytes collision returns `record_id_collision` and cannot replace the retained entry | Property | P0 | FR-057-AC-5 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_204` |
| TC-1209 | Completed, failed, inconclusive, and cause-not-established records remain queryable with byte-identical raw-evidence digests | Unit | P0 | FR-057-AC-6 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_205` |
| TC-1210 | Experiment conclusions render as claims while measured effects and raw references render only as evidence | Unit | P0 | FR-057-AC-7 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_206` |
| TC-1211 | Unknown/uncontrolled interactions and confounders render as counterevidence, and gaps remain separate beside owner and actions | Unit | P0 | FR-057-AC-8 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_207` |
| TC-1212 | Human and JSON output contain no derived overall trust, confidence, or quality score and never infer causality from absent qualifiers | Unit | P0 | FR-057-AC-9, FR-057-CON-2 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_208` |
| TC-1213 | Reordered store input renders byte-identically and human/JSON views expose the same claim-centered sections | Property | P0 | FR-057-AC-10 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_209` |
| TC-1214 | Intake and reporting spawn no experiment or producer process | Static | P0 | FR-057-CON-1 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_211` |
| TC-1215 | Existing measurement collections and pre-experiment evidence remain readable without migration | Integration | P0 | FR-057-CON-3 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_212` |
| TC-1216 | Unsafe, missing, wrong-sized, and digest-mismatched raw evidence returns `raw_evidence_mismatch`, identifies each mismatch, and leaves the record store unchanged | Property | P0 | FR-057-AC-11 | ✅ `quoin-measurement` `tc_479_fr057_criteria.rs` :: `tc_479_210` |
| TC-1217 | Two real reports governed by a supported input-schema declaration and carrying the same scenario ids produce treatment-linked sample counts, pass rates, treatment-minus-baseline effects, and the treatment observation clock | Integration | P0 | FR-058-AC-1 | ✅ `quoin-measurement` `tc_479_fr058_criteria.rs` :: `tc_479_250` |
| TC-1218 | An absent or unsupported input-schema declaration and empty, malformed, structurally incompatible, duplicate, or scenario-mismatched reports are refused without an intervention record | Property | P0 | FR-058-AC-2 | ✅ `quoin-measurement` `tc_479_fr058_criteria.rs` :: `tc_479_251` |
| TC-1219 | Observation time and raw-evidence media type, byte size, and digest derive from exact report bytes and ignore caller-supplied substitutes | Property | P0 | FR-058-AC-3 | ✅ `quoin-measurement` `tc_479_fr058_criteria.rs` :: `tc_479_252` |
| TC-1220 | Inadequate repetition, uncontrolled/unknown qualifiers, and absent attribution method yield `cause_not_established`, none confidence, and explicit gaps | Property | P0 | FR-058-AC-4 | ✅ `quoin-measurement` `tc_479_fr058_criteria.rs` :: `tc_479_253` |
| TC-1221 | A real external two-run evaluation feeds retained reports to the producer, which spawns no process and persists through FR-057 | E2E | P0 | FR-058-AC-5 | ✅ `quoin-measurement` `tc_479_fr058_criteria.rs` :: `tc_479_254` |
| TC-1223 | The v1 envelope requires identities, observation timestamp, deployed scope, record shape, control kind, governance, raw evidence, and every unchanged producer-tuple field | Unit | P0 | FR-059-AC-1 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_300` |
| TC-1224 | Every declared release, deployment, control, fallback, reporting, and version-pinning kind validates while an unknown kind is refused | Unit | P0 | FR-059-AC-2 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_301` |
| TC-1225 | A standing capability requires surface, availability, roles, coverage, limitations, transitions, and an explicit clock-support choice; only supported clocks admit and require event/deadline fields | Unit | P0 | FR-059-AC-3 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_302` |
| TC-1226 | An actual or drill exercise requires ordered timing, actor, trigger, outcome, before/after state, at least one observation, and clock applicability | Unit | P0 | FR-059-AC-4 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_303` |
| TC-1227 | Exactly one of capability and exercise is present for every generated valid record, and adding or removing the discriminator-matched payload fails | Property | P0 | FR-059-AC-5 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_304` |
| TC-1228 | Clocked exercises accept only timestamp-consistent open, met, or missed states; gaps cannot override a deterministic status; not-applicable clocks exclude clock timestamps | Property | P0 | FR-059-AC-6 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_305` |
| TC-1229 | Every pin control requires a matching typed identity, revision, and digest; empty, duplicate, and wrong-kind pin lists fail | Property | P0 | FR-059-AC-7 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_306` |
| TC-1230 | Succeeded, failed, partial, and aborted exercises all round-trip without collapsing their outcome | Property | P0 | FR-059-AC-8 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_307` |
| TC-1231 | Gaps, owner, actions, and safe content-digested raw evidence with media type and byte size are required; invalid capability links and undeclared fields fail | Property | P0 | FR-059-AC-9 | ✅ `quoin-measurement` `tc_479_fr059_criteria.rs` :: `tc_479_308` |
| TC-1232 | Valid intake writes exactly one complete canonical operational record by one atomic same-directory no-replace publication | Integration | P0 | FR-060-AC-1 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_400` |
| TC-1233 | Invalid schema, cross-record, or temporal input returns `invalid_record`; unsafe or byte-mismatched raw evidence returns `raw_evidence_mismatch`; neither writes a store entry | Property | P0 | FR-060-AC-2 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_401` |
| TC-1234 | An absent or mismatched governing definition returns its stable reason code, names requested, expected, and observed versions, and leaves the store unchanged | Unit | P0 | FR-060-AC-3 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_402` |
| TC-1235 | Repeating identical canonical bytes for one id is byte-idempotent for standalone records and records already retained in a pair | Property | P0 | FR-060-AC-4 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_403` |
| TC-1236 | Same-id/different-bytes races at one destination and across standalone/pair containers cannot replace or duplicate the retained logical id; a stale global intake lock fails closed as `intake_busy` | Property | P0 | FR-060-AC-5 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_404` |
| TC-1237 | Both standing capabilities and every exercise outcome remain queryable with byte-identical raw-evidence digests | Unit | P0 | FR-060-AC-6 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_405` |
| TC-1238 | Only a kind, subject, scope, accepted-mode, and exact obligation-clock-condition match with succeeded outcome and timestamp-derived timely completion discharges; every other case names non-discharge | Property | P0 | FR-060-AC-7 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_406` |
| TC-1239 | Only available capabilities and succeeded clock-satisfying exercises render as claims/evidence; unavailable, unknown, not-applicable, adverse, and incomplete states render as counterevidence/gaps | Unit | P0 | FR-060-AC-8 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_407` |
| TC-1240 | Human and JSON output contain no derived overall trust, confidence, or quality score | Unit | P0 | FR-060-AC-9 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_408` |
| TC-1241 | Reordered store input renders byte-identically and human/JSON views expose the same claim-centered sections | Property | P0 | FR-060-AC-10 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_409` |
| TC-1242 | Intake and reporting invoke, drill, or alter no operational control | Static | P0 | FR-060-CON-1 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_410` |
| TC-1243 | Existing measurement collections and intervention evidence remain readable and render alongside operational records without migration | Integration | P0 | FR-060-CON-2 | ✅ `quoin-measurement` `tc_479_fr060_criteria.rs` :: `tc_479_411` |
| TC-1244 | Quoin's retained release workflow and a real completed workflow-run/jobs export produce one linked capability/exercise pair with source-derived workflow, revision, actor, timing, outcome, and observations | Integration | P0 | FR-061-AC-1 | ✅ `quoin-measurement` `tc_479_fr061_criteria.rs` :: `tc_479_450` |
| TC-1245 | Malformed input, workflow/path/event/revision mismatch, and absent, duplicate, unstarted, or incomplete release jobs are refused without either operational record | Unit | P0 | FR-061-AC-2 | ✅ `quoin-measurement` `tc_479_fr061_criteria.rs` :: `tc_479_451` |
| TC-1246 | Workflow structure determines capability state and API exports determine exercise/clock state; caller-supplied substitutes cannot change them | Property | P0 | FR-061-AC-3 | ✅ `quoin-measurement` `tc_479_fr061_criteria.rs` :: `tc_479_452` |
| TC-1247 | Every non-success GitHub conclusion remains a named non-success exercise and cannot discharge a clocked release obligation | Property | P0 | FR-061-AC-4 | ✅ `quoin-measurement` `tc_479_fr061_criteria.rs` :: `tc_479_453` |
| TC-1248 | A real release run is captured outside Quoin, then retained artifacts are consumed with no network/process execution and the linked pair is persisted all-or-nothing through FR-060 | Integration | P0 | FR-061-AC-5 | ✅ `quoin-measurement` `tc_479_fr061_criteria.rs` :: `tc_479_454` |
| TC-1249 | Fan-out emits one row per suite with the sorted distinct live obligations and owners; duplicate symbols and duplicate suite-obligation records do not inflate the count | Property | P0 | FR-062-AC-1 | ✅ `quoin-graph-analysis` `tc_385_parity.rs`, `tc_385_binding_order_is_not_observable.rs` |
| TC-1250 | A binding whose obligation is absent from the accepted export remains named under its suite and in gaps while contributing nothing to the live-obligation count | Unit | P0 | FR-062-AC-2 | ✅ `quoin-graph-analysis` `tc_385_parity.rs` |
| TC-1251 | The exact default and replacement relation sets are recorded; reverse dependency closure reaches transitive and shared dependents, terminates cycles, and selects the lexicographically first shortest path independent of input order | Property | P0 | FR-062-AC-3 | ✅ `quoin-graph-analysis` `tc_385_parity.rs`, `quoin-core` `ops/graph/tests.rs` :: `an_absent_relation_list_is_not_an_empty_one` |
| TC-1252 | Every reached StR, US, FR, or NFR joins all live obligations and suites with depth and path; an unknown or non-requirement seed is a named gap with no partial closure for that seed | Unit | P0 | FR-062-AC-4 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_040` |
| TC-1253 | A reachable supported binding remains supported while change exposure is reported separately, and every other auditor verdict is copied byte-for-byte | Unit | P0 | FR-062-AC-5, FR-062-CON-3 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_041` |
| TC-1254 | One affirmation copied across several suite bindings deduplicates by obligation, actor, commit, and note while retaining every affected suite | Property | P0 | FR-062-AC-6 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_042` |
| TC-1255 | Churn retains zero-event live obligations, sorts count-descending then id, and reports affirmation history for an absent obligation only as a gap | Property | P0 | FR-062-AC-7 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_043` |
| TC-1256 | Every view carries the accepted full source revision and format/module premises byte-for-byte across repeated analysis of identical inputs | Property | P0 | FR-062-AC-8 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_044` |
| TC-1257 | Invalid inputs, absent/unreadable required `--export`/`--premises`/`--audit` paths, non-exact premises, mismatched audit identity, and syntactically valid but malformed bindings fail closed; unavailable retained inputs never appear as a healthy zero | Unit | P0 | FR-062-AC-9 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_045`, `quoin-core` `ops/graph/tests.rs` :: `a_missing_declared_input_is_refused_by_name`, `an_absent_bindings_store_is_a_report_and_not_a_refusal` |
| TC-1258 | Reordered equivalent premises, auditor verdict arrays, and graph inputs emit byte-identical JSON, and human and JSON render from the same sorted report object | Property | P0 | FR-062-AC-10 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_046`, `quoin-core` `ops/graph/tests.rs` :: `the_two_spellings_render_one_report` |
| TC-1259 | A non-JSON command-path and static boundaries reject the inherited update check plus producer, suite, Quire, Git, network, write, frontmatter-reader, and independently built graph dependencies from all three views | Static | P0 | FR-062-AC-11, FR-062-CON-1, FR-062-CON-4 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_047`, `graph-command.test.ts` |
| TC-1260 | Without graph invocation, evidence audit, assurance-case, and measurement goldens stay byte-identical; graph outputs contain no score or threshold classification | Integration | P0 | FR-062-AC-12, FR-062-CON-2 | ✅ `quoin-graph-analysis` `tc_500_restated_criteria.rs` :: `tc_500_048` plus unchanged regression suites |
...
| TC-1293 | The adapter registry exposes the two exact versioned graph adapters, and an unknown name fails with the available names before record parsing | Unit | P0 | FR-066-AC-1 | ✅ `quoin-measurement-graph` `tc_475_parity.rs` |
| TC-1294 | A valid Quire export is preserved field-for-field, while unknown format, module-version, and schema-digest premises each fail before any graph record is returned | Unit | P0 | FR-066-AC-2 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_020` |
| TC-1295 | Standalone and portfolio graph views receive the same authoritative artifact, obligation, symbol, relation, observation, and availability records with no frontmatter read or relation translation | Integration | P0 | FR-066-AC-3, FR-066-CON-3 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_021` |
| TC-1296 | Graph-quality v1 validation recomputes the canonical observation id and refuses changed, missing, or unknown fields before constructing a collection | Property | P0 | FR-066-AC-4 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_022` |
| TC-1297 | The named scorer attachment is retained byte-for-byte with media type and digest; missing or changed bytes leave no collection | Property | P0 | FR-066-AC-5 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_023` |
| TC-1298 | Subject, scope, timestamp, environment, every verification-stack field, and immutable producer identity are independently required and never synthesized | Unit | P0 | FR-066-AC-6, FR-066-CON-2 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_024` |
| TC-1299 | An absent, inactive, wrong-id, or wrong-definition graph-quality plan is refused with expected and observed premises | Unit | P0 | FR-066-AC-7 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_025` |
| TC-1300 | Population totals and every language, node-kind, relation-kind, and resolver-tier census cell map bijectively to sorted count observations | Property | P0 | FR-066-AC-8 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_026` |
| TC-1301 | Every confusion component, unresolved count, ambiguous count, and recall ratio maps bijectively, while two facts mapping to one normalized key are refused | Property | P0 | FR-066-AC-9 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_027` |
| TC-1302 | Empty, unreadable, and unsupported inputs retain valid census rows, emit one distinct not-computed state, and emit no result row | Unit | P0 | FR-066-AC-10 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_028` |
| TC-1303 | Identical adapter intake is byte-idempotent, same-id changed content cannot replace a collection, and direct collection intake is unchanged | Integration | P0 | FR-066-AC-11, FR-066-CON-4 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_029` |
| TC-1304 | Static boundaries reject Quire, extractor, scorer, suite, Git, network, and frontmatter-reader dependencies from the adapters | Static | P0 | FR-066-AC-12, FR-066-CON-1, FR-066-CON-3 | ✅ `quoin-measurement-graph` `tc_480_fr066_criteria.rs` :: `tc_480_030` |
| TC-1305 | Each repository's current graph-quality row carries its active plan, definition, full producer tuple, source/corpus revisions, population identity, and raw record/scorer digests | Unit | P0 | FR-067-AC-1 | ✅ `quoin-measurement-graph` `tc_476_graph_portfolio.rs` :: `tc_476_001` |
| TC-1306 | History retains every structurally readable graph collection in timestamp/id order, including an explicitly incompatible record no longer governed by the active plan | Unit | P0 | FR-067-AC-2 | ✅ `quoin-measurement-graph` `tc_476_graph_portfolio.rs` :: `tc_476_001`, `tc_480_fr067_criteria.rs` :: `tc_480_046` |
| TC-1307 | Every measure/dimension/key partition remains separate under input and repository permutations; no partition or repository is summed or averaged | Property | P0 | FR-067-AC-3 | ✅ `quoin-measurement-graph` `tc_476_graph_portfolio.rs` :: `tc_476_001`, `tc_480_fr067_criteria.rs` :: `tc_480_045` |
| TC-1308 | Missing, unreadable, incompatible, unknown, and not-applicable availability remain distinct from measured/not-computed and from numeric zero | Property | P0 | FR-067-AC-4 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_040` |
| TC-1309 | Matching plans, definitions, configurations, tools, corpus revisions, and population identities permit a delta; changing each one independently blocks it and names the mismatch | Property | P0 | FR-067-AC-5 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_041` |
| TC-1310 | Current, history, and comparison rows resolve to the exact retained producer-record and scorer digests rather than copied summary fields | Integration | P0 | FR-067-AC-6 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_042` |
| TC-1311 | An accepted export/premises/audit triple embeds standalone FR-062 fan-out/churn and requested impact objects byte-for-byte; a partial triple is incompatible without reads, absent inputs are missing, and absent seeds are not-applicable | Integration | P0 | FR-067-AC-7, FR-067-CON-3 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_043`, `tests/graph-portfolio-command.test.ts` |
| TC-1312 | An unreadable repository, collection, attachment, or export is named locally without hiding readable repositories or structurally readable sibling collections in the same store | Unit | P0 | FR-067-AC-8 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_044`, `quoin-measurement` `tc_479_fr044_fr045_criteria.rs` :: `tc_479_017` |
| TC-1313 | Equivalent repository arguments, changed-seed permutations, and store enumeration permutations emit byte-identical canonical JSON; conflicting export, premises, or audit mappings fail before reads; human output renders the same report object | Property | P0 | FR-067-AC-9 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_045` |
| TC-1314 | Historical measurement schemas and non-graph portfolio goldens remain readable without migration, and graph output contains no aggregate score or verdict | Integration | P0 | FR-067-AC-10, FR-067-CON-2, FR-067-CON-4 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_046` |
| TC-1315 | Static boundaries prove portfolio reporting executes and writes nothing and consumes FR-062 reports without recomputing graph semantics | Static | P0 | FR-067-AC-11, FR-067-CON-1, FR-067-CON-3 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_047` |
| TC-1316 | One retained producer fixture passes through the versioned adapter into a portfolio that preserves raw identity, partitions, availability, and compatibility premises while boundary sentinels observe no producer execution and no aggregate verdict | Integration | P0 | StR-007-VC-1 | ✅ `quoin-measurement-graph` `tc_480_fr067_criteria.rs` :: `tc_480_048` |
| TC-1317 | `change-assurance seal-record` seals an explicit body, retains it under its digest, and reports digest and path; a body carrying `digest` or an undeclared field is refused with exit 2 and retains nothing | Unit | P0 | FR-068-AC-1 | ✅ `quoin-core` `tc_457_change_assurance_boundary.rs` :: `tc_457_600` |
| TC-1318 | `change-assurance seal-attestation` derives only retained-output media type, digest, and size from the named file and flag, and refuses a body supplying `retained_output` or `digest` | Unit | P0 | FR-068-AC-2, FR-068-CON-2 | ✅ `quoin-core` `tc_457_change_assurance_boundary.rs` :: `tc_457_601` |
| TC-1319 | `change-assurance intake` retains exact attestation and output bytes as one pair, refuses a contradicting digest or size, is idempotent for identical bytes, and refuses a digest collision with differing bytes | Unit | P0 | FR-068-AC-3 | ✅ `quoin-core` `tc_457_change_assurance_boundary.rs` :: `tc_457_602` |
| TC-1320 | `change-assurance receipt` assembles its verification input only from the named record, parents, explicit selections, decisions, and audits; an unselected stored attestation changes no receipt field | Unit | P0 | FR-068-AC-4 | ✅ `quoin-core` `tc_457_change_assurance_boundary.rs` :: `tc_457_603` |
| TC-1321 | Unavailable, not-computed, not-evaluated, and unselected evidence each survive the command as their own outcomes and reasons, walked exhaustively over that bounded domain with a floor on the distinct reasons reached, and are never rendered as pass or failure | Property | P0 | FR-068-AC-5 | ✅ `quoin-core` `tc_457_change_assurance_boundary.rs` :: `tc_457_604` |
| TC-1322 | Exit status is 0 for a valid receipt, 1 for invalid and incomplete, and 2 for usage, parse, and integrity errors, with the receipt still emitted for the first two | Unit | P0 | FR-068-AC-6 | ✅ `tests/change-assurance-command-surface.test.ts` |
| TC-1323 | `change-assurance verify-receipt` accepts a sealed receipt and refuses one whose semantic field or digest was altered | Unit | P0 | FR-068-AC-7 | ✅ `quoin-core` `tc_457_change_assurance_boundary.rs` :: `tc_457_605` |
| TC-1324 | `change-assurance schema` lists the three normative asset names and emits each byte-identically to the packaged asset; an unknown name is refused with exit 2 | Unit | P0 | FR-068-AC-8 | ✅ `tests/change-assurance-command-surface.test.ts` |
| TC-1325 | `change-assurance recover` removes only interrupted-intake staging directories, reports the count, and leaves retained records, attestations, and outputs untouched | Unit | P0 | FR-068-AC-9 | ✅ `quoin-core` `tc_457_change_assurance_boundary.rs` :: `tc_457_606` |
| TC-1326 | Golden fixtures for a sealed record, a sealed attestation, and a valid receipt reproduce byte-identical canonical JSON through the commands | Integration | P0 | FR-068-AC-10, FR-068-CON-4 | ✅ `tests/change-assurance-command-surface.test.ts` |
| TC-1327 | Static boundaries prove no change-assurance command executes an attested command or performs Git or network work, that the one process boundary any of them reaches is the FR-096 engine module, and that no output or help text makes an identity, authorization, non-repudiation, or certification claim | Static | P0 | FR-068-AC-11, FR-068-CON-1, FR-068-CON-3 | ✅ `tests/change-assurance-command-surface.test.ts` |
...
| TC-1585 | Real producer trace ids retain their exact values and order; omitted legacy metadata remains absent | Unit | P0 | FR-069-AC-9 | ✅ `tests/contract-conformance-traces.test.ts` |
...
| TC-1353 | Under semantic-core `0.2.0`, `quire` fences under `### notArchived` and `### archived` extract to the expected `ClauseRef`s and verbatim text with no diagnostic and a non-lossy clauses kind | Unit | P0 | FR-072-AC-1 | 🚧 fixture published here; no quoin test executes it — agent-ix/quire-rs `tests/semantic_clauses.rs` :: `golden_operations_and_spans` |
| TC-1354 | No language or `tla` fails at the fence; `sysml`, `fretish`, `acme:tla` yield `semantic.clause-language-unchecked` | Unit | P0 | FR-072-AC-2 | 🚧 fixture published here; no quoin test executes it — agent-ix/quire-rs `tests/semantic_clauses.rs` :: `language_cases` |
| TC-1355 | Two `### immutable` clauses fail at the second; `### not-archived` fails at the heading | Unit | P0 | FR-072-AC-3 | 🚧 fixture published here; no quoin test executes it — agent-ix/quire-rs `tests/semantic_clauses.rs` :: `structural_cases` |
| TC-1356 | `### archive` with param table, `Returns: ConfigVersion[1]`, `Requires: notArchived`, `Ensures: archived` yields the expected `OperationDecl` | Unit | P0 | FR-072-AC-4 | 🚧 fixture published here; no quoin test executes it — agent-ix/quire-rs `tests/semantic_clauses.rs` :: `golden_operations_and_spans` |
| TC-1357 | `Ensures: missing` fails at that line | Unit | P0 | FR-072-AC-5 | 🚧 fixture published here; no quoin test executes it — agent-ix/quire-rs `tests/semantic_clauses.rs` :: `operation_cases` |
| TC-1358 | A clause declared by a fence and by `Clause: ./clauses.md#immutable` fails at the second occurrence | Unit | P0 | FR-072-AC-6 | 🚧 fixture published here; no quoin test executes it — agent-ix/quire-rs `tests/semantic_clauses.rs` :: `structural_cases` |
| TC-1359 | No clause typechecking or evaluation code path exists in Quoin | Static | P0 | FR-072-CON-1 | 🚧 `quoin-semantic` `tc_452_sweep_criteria.rs` :: `tc_452_653` fails on main: its module census omits `embedded` |
| TC-1715 | The `fence-ocl-carried` case (semantic-core `0.2.0`) and the `config-version` golden (`0.1.0`) each carry the `ocl` body verbatim with one `semantic.clause-language-unchecked` advisory at the fence and a lossy clauses kind | Unit | P0 | FR-072-AC-7 | 🚧 fixtures published here; no quoin test executes them — agent-ix/quire-rs `tests/semantic_clauses.rs` :: `quire_is_checked_and_ocl_is_carried` (0.2.0), `golden_operations_and_spans` (`config-version`, 0.1.0) |
| TC-1716 | Under semantic-core `0.1.0`, the `fence-quire-invalid` case yields `semantic.clause-language-invalid` at the fence and an unavailable clauses kind | Unit | P0 | FR-072-AC-8 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#432 adds `clause-language-0.1.0-cases.json` to `tests/semantic_clauses.rs` :: `quire_is_checked_and_ocl_is_carried` |
| TC-1717 | `relationships.md` extracts to the expected `RelationDecl[]` (registry category, `part_of` composite, bundle-qualified targets, unbounded `0..*`) with a row-line `relationSources` span per element, each validating against `RelationDecl.json` | Unit | P0 | FR-104-AC-1 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1718 | `belongs_to` and the inverse label `part_of` fail as `unknown-verb`; `depends_on` fails as `verb-not-allowed` | Unit | P0 | FR-104-AC-2 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1719 | A title, an undeclared id, and an unimported `ix://` target fail as `target-not-id`; an imported `ix://` target is carried verbatim | Unit | P0 | FR-104-AC-3 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1720 | Multiplicity cells `one`, `5..2`, and empty fail as `multiplicity` at the row | Unit | P0 | FR-104-AC-4 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1721 | `config-overlay` fails as `name-not-identifier`; a repeated `Name` yields `semantic.duplicate-model-entry` at the second row and unavailable relations | Unit | P0 | FR-104-AC-5 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1722 | A missing or extra column yields `semantic.feature-not-extractable` at the header with `section` `Relationships` | Unit | P0 | FR-104-AC-6 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1723 | A bullet list under `## Relationships` is refused at its first line; a second table yields `semantic.duplicate-section` | Unit | P0 | FR-104-AC-7 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1724 | Without the `relationships` mapping a conforming table is refused at its header and a bullet list is prose with no diagnostic | Unit | P0 | FR-104-AC-8 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1725 | Category and composition come only from the registry entry, never from verb spelling, target name, or roles | Static | P0 | FR-104-CON-1 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
| TC-1726 | The relationships goldens record semantic-core `0.2.0` and the registry and bundle context they assume | Static | P0 | FR-104-CON-2 | 🚧 fixture published here; no test executes it yet — agent-ix/quire-rs#418 |
...
| TC-277 | `/specify` names all three installed assurance artifact types, obtains them through the live `quoin write` contract, and teaches only schema-admitted relationships | Static | P1 | FR-095-AC-1 | ✅ |
| TC-278 | `/specify` states the opt-in and stop boundaries: an ordinary request gets no unsolicited assurance artifact and no review is launched without confirmation | Static | P0 | FR-095-AC-2 | ✅ |
| TC-279 | A profile recommendation remains advisory: a user choosing base records no selected analyses and passes the existing zero-analysis path | Unit | P0 | FR-095-AC-3, FR-095-AC-8 | ✅ |
| TC-280 | A required profile selection refuses a missing profile path or any selected set unequal to the profile set | Unit | P0 | FR-095-AC-4 | ✅ |
| TC-281 | The all set always expands to the canonical analysis vocabulary; a caller-supplied truncated list fails intake and cannot weaken the final coverage gate | Unit | P0 | FR-095-AC-5 | ✅ |
| TC-282 | A required profile analysis absent from `review_doc` is named by the final coverage gate and prevents acceptance | Unit | P0 | FR-095-AC-6 | ✅ |
| TC-283 | Direct skill, workflow documentation, intake, invariant, and installed `SpecReview.analysis` schema remain aligned; a profile value outside that set is refused as unsupported | Unit | P0 | FR-095-AC-7 | ✅ |
| TC-284 | With no applicable profile, the base review path retains the existing empty selected set and passes | Unit | P0 | FR-095-AC-8 | ✅ |
| TC-EV-058 | In both Claude and Codex harnesses, an explicit assurance-profile request fetches the installed authoring contract once, produces exactly one linked and validated profile, creates no other assurance/review artifact, and stops before review | Eval | P0 | FR-095-AC-1, FR-095-AC-2 | ✅ Paired live reports preserved in `agent-ix/engineering-assurance@2239962` |
| TC-EV-059 | In both Claude and Codex harnesses, an ordinary contained FR request with the assurance module installed produces exactly one validated FR and no unsolicited assurance/review artifact | Eval | P0 | FR-095-AC-2 | ✅ Paired live reports preserved in `agent-ix/engineering-assurance@2239962` |
...
| TC-1428 | With the engine absent, the rendered suite fails naming the provisioning command and the tracking issue | Unit | P0 | FR-080-AC-1 | 🚧 the failure path is authored in the rendered conftest; no run yet removes the engine and observes it (agent-ix/quoin#346) |
| TC-1429 | With an engine lacking `extract_semantic`, the rendered suite fails naming the missing capability | Unit | P0 | FR-080-AC-2 | 🚧 the failure path is authored in the rendered conftest; no run yet installs an engine without `extract_semantic` (agent-ix/quoin#346) |
...
| TC-1431 | The rendered gate runs spec validation, lint, the schema drift check, and the suite, and fails when any one fails | Integration | P0 | FR-080-AC-5 | 🚧 the rendered `make gate` runs the four legs; no run yet fails one leg and asserts the target fails (agent-ix/quoin#346) |
...
| TC-1448 | With the schema toolchain absent, the gate fails naming the install command and reports no skipped emission check | Unit | P0 | FR-083-AC-6, NFR-020-AC-3 | 🚧 the emit driver names the install command when the toolchain is absent; no run yet removes it and observes the failure (agent-ix/quoin#346) |
| TC-1449 | A deliberate TypeSpec error fails emission with the compiler diagnostics in the message and changes no committed schema, toolchain, or manifest byte | Unit | P0 | FR-077-AC-7 | 🚧 no run yet compiles a deliberately broken `main.tsp` in a rendered tree (agent-ix/quoin#346) |
| TC-1450 | With the semantic-core grammar package absent, the rendered suite fails naming the install command and reports no skipped schema check | Unit | P0 | FR-080-AC-7 | 🚧 the failure path is authored in the rendered conftest; no run yet removes the grammar package and observes it (agent-ix/quoin#346) |
...
| TC-1457 | A ref to an imported model is written against that module's base at the imported version, and a ref matching no declared base fails emission naming it | Unit | P0 | FR-077-AC-8 | 🚧 the mixed variant emits with an import declared; no run yet writes a cross-package `$ref` or an unresolvable one (agent-ix/quoin#346) |
...
| TC-1515 | A `type` declared by two resolved modules yields `contested` naming both, no aggregate reassigns it, and one `contract-defect` finding names the contesting modules | Unit | P0 | FR-086-AC-4 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1518 | A document the engine reports no error against records `pass` | Unit | P0 | FR-087-AC-1 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1519 | A document the engine reports a missing required section against records `fail` carrying the engine's diagnostic code, reason and line | Unit | P0 | FR-087-AC-2 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1520 | A repository-local module declaring the same type has no effect on the outcome; the resolved module set is the contract applied | Integration | P0 | FR-087-AC-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1521 | A batch whose engine invocation terminates abnormally records every document of that batch `could-not-run` and none enters either side of the rate | Unit | P0 | FR-087-AC-4, FR-087-CON-1 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1522 | A document the engine reports only warnings against records `pass` and carries those warnings as advisory findings | Unit | P0 | FR-087-AC-5 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1523 | A document carrying three engine errors records all three in one evaluation record | Unit | P1 | FR-087-AC-6 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1524 | Changing only the module declaration in the resolved set changes the recorded outcome of an unchanged document | Integration | P0 | FR-087-AC-7 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1526 | The form census counts documents, states that unit, and its per-form counts sum to the enumerated document count | Property | P0 | FR-088-AC-2 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1527 | A free-column table and a bullet list each yield one `unsupported-representation` advisory finding and neither is counted as a conformance failure | Unit | P0 | FR-088-AC-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1528 | A document with no `## Properties` heading records `not-applicable` and is absent from both sides of the rate | Unit | P0 | FR-088-AC-4, FR-088-CON-1 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1529 | Every document's field-level record is `could-not-run` carrying a tool-defect citation while the toolchain record shows no semantic-extraction surface | Unit | P0 | FR-088-AC-5 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1530 | The document set the form census classified equals the set FR-084 enumerated, element by element | Property | P0 | FR-088-AC-6, FR-088-CON-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1531 | Every reported finding appears in exactly one partition entry and the counts agree | Property | P0 | FR-089-AC-1, FR-089-CON-2 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1532 | A finding no ledger entry matches is `unknown`/`undispositioned` and both counts are headline figures | Unit | P0 | FR-089-AC-2 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1533 | A `tool-defect` ledger entry without a repository and issue number is refused and its findings stay `unknown` | Unit | P0 | FR-089-AC-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1534 | A disposition missing its required nomination — later campaign, module repository, or accepting human — is refused | Unit | P0 | FR-089-AC-4 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1536 | `contract-defect` requires a ledger entry recording that the declaration is wrong; without one the finding stays `unknown` | Unit | P0 | FR-089-AC-6 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1537 | Per-class counts sum to the reported finding count | Property | P0 | FR-089-AC-7, FR-089-CON-1 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1539 | Every rate's population identifier names the corpus identifier, every repository commit, every module commit and the engine source revision | Unit | P0 | FR-090-AC-2 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1541 | The structural rate and the form census are published separately with their own populations and units and are never summed | Unit | P0 | FR-090-AC-4 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1544 | Every declared type appears in the by-type breakdown keyed on module and type; a type with no instance carries denominator zero and no rate value | Property | P0 | FR-090-AC-7 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1552 | Every enumerated corpus repository's working tree and refs are byte-identical before and after a run | Integration | P0 | FR-092-AC-2, FR-092-CON-1, FR-084-CON-1, FR-087-CON-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1556 | An unresolvable required module exits non-zero before any corpus document is read | Unit | P0 | FR-092-AC-6, FR-092-CON-2, FR-085-AC-4 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1566 | A symbolic link to a directory is not traversed, is recorded under the rule `symlink`, contributes no document, and a link cycle terminates the walk | Unit | P0 | FR-084-AC-7 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1567 | Changing only the declared exclusion vocabulary changes the enumerated document count | Integration | P0 | FR-084-AC-8, FR-084-CON-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1568 | A `data_schema` digest disagreeing with the schema file read yields a module finding naming both digests | Unit | P0 | FR-085-AC-7 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1569 | The toolchain record names the engine and CLI versions and source revisions and each module's `semantic_core` version | Unit | P0 | FR-085-AC-8 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1570 | A module-load diagnostic is recorded as a module finding and fails no corpus document | Unit | P0 | FR-085-AC-9 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1571 | A required module set missing one completed module is refused before any corpus document is read | Unit | P0 | FR-085-AC-10, FR-085-CON-2, FR-085-CON-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1572 | The evaluation output names the engine version and source revision its outcomes were produced by | Unit | P0 | FR-087-AC-8 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1573 | A `type` differing from a declared type only by letter case is `out-of-model`, not `measured` | Unit | P0 | FR-086-AC-7 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1574 | Two modules declaring one type name yield two distinct type keys and neither's documents are attributed to the other | Unit | P0 | FR-086-AC-8, FR-086-CON-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1575 | Moving a finding's document line keeps its ledger match; changing its diagnostic code returns it to `unknown` | Unit | P0 | FR-089-AC-8 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1576 | A ledger entry matching no finding appears in `unmatched-ledger-entries` and in the headline count | Unit | P0 | FR-089-AC-9 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1577 | `malformed-document`, `missing-structure` and `stale-module` are each produced only from a ledger entry recording that condition | Unit | P0 | FR-089-AC-10 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1578 | Every figure the prose report prints appears in the machine-readable index naming its artifact and field | Property | P0 | FR-090-AC-8, NFR-023-AC-1 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1579 | Changing only the declared divergence margin changes the membership of the divergence list | Unit | P0 | FR-090-AC-9, FR-090-CON-3 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1580 | The count of repositories recorded `clean: false` or `stable: false` is published beside every corpus-level rate | Unit | P1 | FR-090-AC-10 | 🚧 not yet implemented (agent-ix/quoin#291) |
| TC-1581 | Every tool-defect citation resolves to an existing issue in the named repository | Static | P1 | FR-091-AC-7, FR-091-CON-1 | 🚧 not yet implemented (agent-ix/quoin#291) |
...
| TC-1601 | Every first-party executable path in this repository carries one current state and one final disposition — port | Unit | P0 | StR-009-VC-2 | 🚧 |
| TC-1602 | Differential qualification preserves accepted success, non-success, malformed, refusal, canonicalization and digest | Unit | P0 | StR-009-VC-3 | 🚧 |
| TC-1603 | No type crossing the boundary is hand-written where `filament-core-data` or a repository schema publishes it, and | Unit | P0 | StR-009-VC-4 | 🚧 |
| TC-1604 | Completion evidence identifies no unapproved first-party non-Rust engine, production, planning, validation | Unit | P0 | StR-009-VC-5 | 🚧 |
| TC-1605 | `cargo build --workspace --manifest-path rust/Cargo.toml` produces the `quoin-core` binary from a workspace inside | Unit | P0 | FR-096-AC-1 | 🚧 |
| TC-1606 | Every boundary operation emits exactly one declared-version JSON document on stdout and sends all diagnostics to | Property | P0 | FR-096-AC-2 | 🚧 |
| TC-1607 | Unknown protocol version, an unknown operation, malformed JSON and an escaping root each refuse before any filesystem | Property | P0 | FR-096-AC-3 | 🚧 |
| TC-1608 | Refusal carrying a valid result payload is distinguishable by the caller from a fault carrying no payload, for every | Unit | P0 | FR-096-AC-4 | 🚧 |
...
| TC-1622 | Each ported refusal path returns the same non-success classification from the Rust implementation as from the | Property | P0 | FR-098-AC-5 | 🚧 |
| TC-1623 | Every `tests/props` property has a `proptest` counterpart asserting the same property, and a property holding in | Unit | P0 | FR-098-AC-6 | 🚧 |
| TC-1624 | Differential run whose compared population is zero is reported as inconclusive and refuses to satisfy a cutover gate. | Unit | P0 | FR-098-AC-7 | 🚧 |
| TC-1625 | `quoin-core validators.run` returns the same verdict and non-success classification as the retained validator path | Unit | P0 | FR-099-AC-1 | 🚧 |
| TC-1626 | `quoin-core quire.validate` returns the same verdict as the retained `src/quire` adapter for every corpus document | Unit | P0 | FR-099-AC-2 | 🚧 |
| TC-1627 | `quoin-core catalog.list` assembles a catalog identical to the retained catalog for the same configuration root | Unit | P0 | FR-099-AC-3 | 🚧 |
| TC-1628 | Second reconcile over an already-materialized configuration root performs no git or network access and yields a | Unit | P0 | FR-099-AC-4 | 🚧 |
| TC-1629 | Manifest carrying an unknown key is refused by the Rust reader with the same classification as the retained reader. | Unit | P0 | FR-099-AC-5 | 🚧 |
| TC-1630 | `quoin-core semantic.validate` and `quoin-core completeness.analyze` return verdicts identical to the retained | Unit | P0 | FR-099-AC-6 | 🚧 |
| TC-1631 | Rust skill tests read the shipped Markdown skills and fail on a planted vocabulary drift, and the binary resolves the | Unit | P0 | FR-099-AC-7 | 🚧 |
| TC-1632 | `package.json` declares no dependency on `@agent-ix/ix-cli-core`, and each imported symbol recorded in the baseline | Unit | P0 | FR-099-AC-8 | 🚧 |
| TC-1633 | `cargo tree --manifest-path rust/Cargo.toml` shows no dependency cycle, and the two named TypeScript cycles are | Unit | P0 | FR-100-AC-1 | 🚧 |
| TC-1634 | Reading and re-serializing every store reachable in the ecosystem through `quoin-store` returns byte-identical | Unit | P0 | FR-100-AC-2 | 🚧 |
| TC-1635 | Each change-assurance refusal for a stale, tampered or mismatched record returns the same classification from the | Property | P0 | FR-100-AC-3 | 🚧 |
| TC-1636 | Each measurement report produced by the Rust crates carries the same states, failure partitions, unit, population and | Unit | P0 | FR-100-AC-4 | 🚧 |
| TC-1637 | Measurement run over a read-only governed corpus completes and the corpus working tree is unchanged afterwards. | Unit | P0 | FR-100-AC-5 | 🚧 |
| TC-1638 | Every locally retained assurance capability carries a recorded three-part retention answer, and a capability with no | Unit | P0 | FR-100-AC-6 | 🚧 |
| TC-1639 | Each enforcement run writes the three classification counts into the existing evidence store as one measurement | Unit | P0 | FR-100-AC-7 | 🚧 |
| TC-1640 | Static check over a named non-empty population of crate sources finds canonical JSON, JCS, sha256 record identity and | Unit | P0 | FR-100-AC-8 | 🚧 |
| TC-1641 | Every first-party executable-path row records one current state and one final disposition, and the gate fails when a | Unit | P0 | FR-101-AC-1 | 🚧 |
| TC-1642 | Removal is refused unless the retained and Rust paths both pass at the same candidate revision and the direct | Property | P0 | FR-101-AC-2 | 🚧 |
| TC-1643 | Reverting a cutover change restores the retained invocation, and comparing the evidence and accepted-corpus trees | Unit | P0 | FR-101-AC-3 | 🚧 |
| TC-1644 | For each deleted test, the criterion it carried resolves to a tracking-tagged Rust test, and mutating the | Unit | P0 | FR-101-AC-4 | 🚧 |
| TC-1645 | After a capability's cutover, no test in the repository spawns or imports a non-Rust implementation of that | Unit | P0 | FR-101-AC-5 | 🚧 |
...
| TC-1658 | Corpus accounting is obtained from `engineering-assurance`, and no local corpus-accounting implementation exists in | Unit | P0 | FR-103-AC-4 | 🚧 |
| TC-1659 | Sample input declared inert in the allowance manifest is reported as an allowance, and the same file moved to an | Unit | P0 | FR-103-AC-5 | 🚧 |
| TC-1660 | Every retained path is emitted in the retained-with-successor class naming an open sub-issue of the burn-down epic | Unit | P0 | NFR-024-AC-1 | 🚧 |
| TC-1661 | Planted retention with no successor, one whose successor is the epic itself or a closed issue, and one whose expiry | Unit | P0 | NFR-024-AC-2 | 🚧 |
| TC-1662 | No report produced by this programme records a retained path as remediated, and no report records the existence of a | Unit | P0 | NFR-024-AC-3 | 🚧 |
| TC-1663 | Standing-approved user-interface TypeScript and `filament-core-data`-published types are emitted in the allowed class | Unit | P0 | NFR-024-AC-4 | 🚧 |
| TC-1664 | Enforcement run that classifies an empty population is reported as inconclusive rather than as zero violations. | Unit | P0 | NFR-024-AC-5 | 🚧 |
| TC-1665 | Digest replay over every reachable store reports zero mismatches and names the non-empty population it covered. | Unit | P0 | NFR-025-AC-1 | 🚧 |
| TC-1666 | Planted single-byte canonicalization difference makes the digest replay fail. | Unit | P0 | NFR-025-AC-2 | 🚧 |
...
| TC-1677 | Each crate boundary exposes exactly one `thiserror`-derived error type whose every code appears in that crate's | Unit | P0 | NFR-027-AC-4 | 🚧 |
| TC-1678 | No public library signature returns `Box<dyn Error>` or an `anyhow` error, and a planted one fails the gate. | Unit | P0 | NFR-027-AC-5 | 🚧 |
| TC-1679 | No public library signature takes or returns a bare `String` or `Uuid` as an identifier or resource reference, and a | Unit | P0 | NFR-027-AC-6 | 🚧 |
| TC-1680 | Every Rust test function carries a tracking tag whose cited criterion ids resolve to an artefact under `spec/`, and | Unit | P0 | NFR-027-AC-7 | 🚧 |
| TC-1681 | Continuous-integration change that removes a gate lane, narrows one, or adds `continue-on-error` fails this | Unit | P0 | NFR-027-AC-8 | 🚧 |
| TC-1682 | Each of the source and test checks reports the population it scanned, and a check that scanned nothing is reported as | Unit | P0 | NFR-027-AC-9 | 🚧 |
| TC-1683 | Every asset under `skills/**/workflow-assets/**` carries a recorded disposition that distinguishes the first-party | Unit | P0 | FR-099-AC-9 | 🚧 |
| TC-1684 | Port or delete disposition recorded against a path inside the `corpus/` submodule fails the gate, naming the | Unit | P0 | FR-103-AC-6 | 🚧 |
| TC-1685 | Submodule is pinned to an exact revision, and a revision change is reported as a corpus identity change with both | Unit | P0 | FR-103-AC-7 | 🚧 |
| TC-1686 | `.language-allowances.yaml` is checked in at the repository root, every entry declares a category, a path or glob and | Unit | P0 | NFR-024-AC-6 | 🚧 |
| TC-1687 | Every `generated` entry declares a generator identity and a source-schema digest, and a hand edit of a covered file | Unit | P0 | NFR-024-AC-7 | 🚧 |
...
| TC-1699 | Interrupted record write leaves no torn record readable and no orphaned temporary file after the next run, and | Unit | P0 | FR-100-AC-9 | 🚧 |
...
| TC-1707 | Classified population includes executable paths with no governed extension, and a planted non-Rust assertion in a | Unit | P0 | NFR-024-AC-12 | 🚧 |
| TC-1708 | Expired retention fails the enforcement report while the build lane still completes, so a calendar boundary does not | Unit | P0 | NFR-024-AC-13 | 🚧 |
| TC-1709 | Published-type integration end to end: same recorded provenance, generated surface matches its asserted digest, an instance round-trips byte-identically, a planted duplicate declaration is refused | Integration | P0 | IT-003 | 🚧 |
| TC-1710 | Manifest entry declaring a category this requirement does not admit fails the run, naming the entry and the category. | Unit | P0 | NFR-024-AC-14 | 🚧 |
| TC-1713 | Over one materialised repository, the verdict reached through the caller's snapshot equals the verdict `quoin-core` reaches reading the same tree from disk, and narrowing the caller's pre-filter by one directory fails the comparison. | Integration | P0 | FR-096-AC-8 | ✅ |
| TC-1714 | A stdin stream one byte past the transport bound is refused without being parsed and without being read beyond that bound, a stream of exactly the bound is accepted, and the transport bound exceeds every per-operation request bound. | Unit | P0 | FR-096-AC-9 | ✅ |
...
| US-003   | ⚠️ Partial | `quoin-modules` `install_reconcile.rs` :: `tc_381_280` + `index.test.ts` install/list/remove (path source only). GitHub/subdir install is exercised only at the eval layer (TC-EV-003/TC-EV-009/TC-EV-010/TC-EV-020). |
| US-022   | ⛔ Withdrawn | Withdrawn 2026-09-12 under [#388](https://github.com/agent-ix/quoin/issues/388), with every requirement it drove (FR-084..FR-092). Gate [#291](https://github.com/agent-ix/quoin/issues/291) (advisory, report only) is closed and the measurement it asked for ran once on 2026-09-05, results retained at `analysis/corpus-measurement/`. Capability class is corpus accounting, which the implementation-language policy places in `engineering-assurance`. Deliberately **not** restated as EA-consumed while [engineering-assurance#98](https://github.com/agent-ix/engineering-assurance/issues/98) is open. |
...
| US-018   | ✅ Covered | The retained module was deleted at [quoin#500](https://github.com/agent-ix/quoin/issues/500); TC-1249..TC-1260 cover exact fan-out, dependency closure, unchanged auditor verdicts, reaffirmation churn, unavailable inputs, deterministic rendering, and read-only/non-scoring boundaries in the `quoin-graph-analysis` `tc_385_*` and `tc_500_restated_criteria.rs` tests, the `quoin-core` `ops/graph` tests, and `graph-command.test.ts`. |
| US-017   | ✅ Covered | TC-1261..TC-1292 cover revisioned definitions, exact integrity, workflow decisions, candidate-bound attestations, retained output, unchanged evidence-audit findings, valid/invalid/incomplete receipts, and non-identity boundaries in the `quoin-change-assurance` `tc_455_*` tests, every one of them replayed against the capture the deleted TypeScript wrote. |
| US-019   | ✅ Covered | The three retained modules were deleted at [quoin#480](https://github.com/agent-ix/quoin/issues/480); `quoin-measurement-graph` `tc_475_parity.rs`, `tc_476_graph_portfolio.rs`, `tc_477_graph_loader.rs`, `tc_480_fr066_criteria.rs` and `tc_480_fr067_criteria.rs`, with `tests/graph-portfolio-command.test.ts`, TC-1293..TC-1315 cover lossless producer intake, plan/definition/population governance, raw evidence, graph history and partitions, availability, compatible comparisons, exact FR-062 portfolio views, and non-scoring. |
...
| US-024 | ⚠️ Spec-ahead-of-code | EX-1..EX-5 are realised by the requirements this story drives, not by tests of its own: reversible cutover and deletion-last by FR-101, the empty-population rule by NFR-024 and NFR-027, coexistence visible as debt by NFR-024, and the approved-TypeScript exclusion by StR-009 and the allowance manifest. Acceptance examples are illustrative and take no direct binding; this row is derived from those requirements, all of which are spec-ahead-of-code. The port has not started (#373). |
...
Five criteria of the corpus measurement are verified by inspection of the
measurement's own source rather than by a test, because each asserts the absence
of code: FR-087-AC-9 and FR-087-CON-2 (no parser, extractor or record builder of
its own), FR-088-AC-7 and FR-088-CON-2 (no `Type`, multiplicity or constraint
resolution of its own), and FR-089-CON-3 (no disposition relaxes a declared
constraint to lower a count). A test can witness the presence of behaviour; it
cannot witness that a second implementation was never written.
...
| IT-002           | ⚠️ Spec-ahead-of-code | `github://` plugin install. Unit tests cover path sources only (`quoin-modules` `install_reconcile.rs`); live GitHub install is exercised at the eval layer (TC-EV-003/TC-EV-009/TC-EV-010/TC-EV-020).                                                    |
| IT-003 | ⚠️ Spec-ahead-of-code | TC-1709 — Quoin ↔ filament-core-data published-type integration. One end-to-end procedure, none implemented: the port has not started. Spec-ahead-of-code by design (#373). |
