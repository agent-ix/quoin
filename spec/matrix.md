...
| FR-047 | ✅ Covered | `tests/independence.test.ts` — TC-303..TC-309. Strict lineage/policy boundaries, current-obligation validation, distinct-evidence pairing, shared/missing lineage, absent-policy compatibility, audit finding clearance, binding persistence/clearing, command reachability, and assurance-context rendering are covered. CON-1/CON-3/CON-4 → inspection of `src/evidence/independence.ts` and the two policy-consuming commands (no role/tool/vendor inference, score, or Markdown parser). CON-2 → TC-306. |
