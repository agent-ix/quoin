...
Tests live in `tests/` and run under vitest (`make test` → `vitest run`). The
Rust workspace's tests live in `rust/crates/*/tests/` and `#[cfg(test)]`
modules and run under `make rust-test`; their tracking tags are `///` doc
comments, and `scripts/check-trace-tags.mjs` scans that tree as a population of
its own with its own empty guard, because a combined guard is vouched for by
the 752 TypeScript tags and can never fail (#390).
...
| FR-096 | 🚧 Partly | TC-1605..TC-1611 — the versioned `quoin-core` boundary — subprocess and JSON over stdin/stdout, command-shaped operations, the 0/1/2/3/4 exit taxonomy with a non-zero-but-valid payload distinguishable, and no Node type crossing. 11 criteria (7 AC, 4 CON); **2 carry test evidence** after #390 rebound Stage 0's tags in `rust/crates/quoin-core/tests/tc_boundary.rs`, which until then named bare requirement ids and bound nothing. AC-2 is backed for the one operation that exists — one declared-version JSON document on stdout, stderr empty. AC-4 is backed at exit 1 and exit 2 only, **not** at 3 or 4, so its "for every declared non-zero exit status" half is open. The other 9 have no test. `tc_375_every_invalid_shape_exits_3_with_its_own_code` is deliberately untagged: it asserts exit 3 for malformed JSON and an unknown operation, while this requirement declares 2 for a malformed or unsupported request and 3 for an unavailable host — `protocol::Outcome` and FR-096 disagree, and one of them has to move. |
| FR-097 | ⚠️ Spec-ahead-of-code | TC-1612..TC-1617, TC-1691 — the schema-sourced type surface — `schemars` emits JSON Schema, a build step generates `src/core/types.ts` hash-asserted, and a hand-written duplicate of a published type is refused. 10 criteria, none implemented: the port has not started. Spec-ahead-of-code by design (#373). |
...
| FR-101 | ⚠️ Spec-ahead-of-code | TC-1641..TC-1648, TC-1690, TC-1700..TC-1702 — retiring replaced executable paths — retain until parity at one candidate revision, a reversible cutover, deletion last, and expected fixtures captured from the retained implementation before it goes. 17 criteria, none implemented: the port has not started. Spec-ahead-of-code by design (#373). |
...
| NFR-026 | ⚠️ Spec-ahead-of-code | TC-1670..TC-1673 — the Rust 1.98.1 toolchain floor, consistent with quire-corpus and filament-core-data. 4 criteria, **none backed**. `tc_375_the_pinned_channel_and_the_declared_msrv_agree` backs the metric row NFR-026-M-2 (declared channel 1.98.1) and nothing else; it is deliberately not tagged AC-1, whose "declared in exactly one file" half the tree does not satisfy — the channel is pinned twice more in `.github/workflows/build-test.yml`, which AC-3 says a gate must reject. |
| NFR-027 | 🚧 Partly | TC-1674..TC-1682 — Rust implementation idioms and the gates that must actually run — a gate described and not run is a green result for a check that did not execute. 9 criteria; **1 carries test evidence**: `tc_375_every_crate_opts_into_the_workspace_lint_policy` backs AC-2's population half (every crate manifest opts into `[lints] workspace = true`) and the metric NFR-027-M-2; AC-2's planted-per-crate-override half is not demonstrated. The other 8 have no test. AC-7 — every Rust test function carries a tracking tag whose criterion ids resolve — is the criterion this row's own defect belonged to: `scripts/check-trace-tags.mjs` now scans `rust/**/*.rs`, reads `///` tags and guards the Rust population separately from the TypeScript one (#390), but it still checks only the tags that exist, not that every test has one, and it remains deliberately unwired from the `Makefile` until the [#350](https://github.com/agent-ix/quoin/issues/350) verification-stack relock. |
