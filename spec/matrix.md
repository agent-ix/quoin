...
| FR-076      | 🚧 In progress | `tests/semantic-module-template.test.ts` TC-1400..TC-1407, TC-1412 — the three variants render from one core, the shared surfaces are single-sourced, and each rejected input aborts naming the value. Implementation lands under agent-ix/quoin#307. |
| FR-077      | 🚧 In progress | TC-1408, TC-1409, TC-1413..TC-1416 — emission, no `{type: object}` schema, byte-determinism, check mode, base/version disagreement, and the semantic-core import. |
...
| FR-080      | 🚧 In progress | TC-1427..TC-1431 — engine-absent failure naming the provisioning path, missing `extract_semantic`, zero skips, no declared engine dependency, and the rendered gate. |
...
| TC-1402 | `module_kind: mixed` renders both sections, and `semantic.imports` carries each imported package identity at an exact version while an object rendering with none yields `imports: {}` | Unit | P0 | FR-076-AC-3, FR-078-AC-5 | 🚧 template not yet landed (agent-ix/quoin#307) |
| TC-1403 | Every file byte-identical across the three rendered variants exists exactly once in the template source, and two renders of one variant are byte-identical | Property | P0 | FR-076-AC-4 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1414 | Emission check mode exits zero on the committed output and non-zero naming the file after one emitted byte changes | Unit | P0 | FR-077-AC-4 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1417 | Each rendered `semantic` block carries the nine admitted keys and no other, validates against the vendored module-manifest schema, and its `exports` equal the declared type names | Unit | P0 | FR-078-AC-1, FR-078-AC-6, FR-078-CON-1 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1419 | Regenerating the manifest digests preserves every comment and YAML anchor in the rendered manifest | Unit | P1 | FR-078-AC-4 | 🚧 template not yet landed (agent-ix/quoin#307) |
| TC-1420 | Every exported type has a skeleton whose Properties table header is exactly the typed four-column form (Field, Type, Multiplicity, Constraints) with at least one row | Unit | P0 | FR-079-AC-1 | 🚧 template not yet landed (agent-ix/quoin#307) |
| TC-1421 | Every rendered skeleton has a `sysml`-fence alternate declaring the same field names, types, and multiplicities | Unit | P0 | FR-079-AC-2 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1423 | Every rendered skeleton carries at least one `ocl` fence under its own `### <clauseId>` heading | Unit | P0 | FR-079-AC-3 | 🚧 template not yet landed (agent-ix/quoin#307) |
| TC-1424 | Every rendered negative fixture is refused, and each for its own distinct reason | Integration | P0 | FR-079-AC-4 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1426 | For artifact and mixed renderings, every property of every exported model has exactly one mapping entry and every golden record round-trips from its skeleton | Integration | P1 | FR-079-AC-8 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1430 | The rendered package metadata declares no dependency on the engine, and the rendered test configuration turns warnings into errors | Static | P0 | FR-080-AC-4, FR-080-AC-6 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1436 | No rendered file matches a credential, token, or private-registry publication pattern, and the npm publication target is the public registry with public access | Static | P0 | FR-081-AC-4, FR-081-AC-5 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1439 | `quire validate` over each rendered variant's `spec/**/*.md` exits zero with no error diagnostic | Integration | P0 | FR-082-AC-1 | 🚧 template not yet landed (agent-ix/quoin#307) |
| TC-1440 | No rendered Test Matrix cell carries the retired `⚠️` marker, every `Status` cell matches the archetype pattern, and an injected `⚠️` fails the gate | Static | P0 | FR-082-AC-2, FR-082-CON-1, FR-083-AC-7 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1442 | Every rendered Test Matrix row traces to an acceptance criterion that exists in the rendered spec | Static | P0 | FR-082-AC-4 | 🚧 template not yet landed (agent-ix/quoin#307) |
...
| TC-1447 | Removing a surface the conformance contract names fails the gate naming that surface | Unit | P0 | FR-083-AC-4 | 🚧 template not yet landed (agent-ix/quoin#307) |
| TC-1448 | With the schema toolchain absent, the gate fails naming the install command and reports no skipped emission check | Unit | P0 | FR-083-AC-6 | 🚧 template not yet landed (agent-ix/quoin#307) |
