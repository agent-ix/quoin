...
| TC-974 | A language set yields one scorable case per language, each with its own suffixed id and its own input tree — the id is the join key across runners, and three variants sharing one are indistinguishable in every join (#242) | Unit | P0 | FR-043-AC-14 | ✅ |
| TC-975 | A case authored into BOTH layouts fails naming the directory, rather than being read as one of them and silently dropping the other (#242) | Unit | P0 | FR-043-AC-14 | ✅ |
| TC-976 | A case authored into NEITHER layout fails: a fixture that scores nothing and a fixture that is not there are not the same fact (#242) | Unit | P0 | FR-043-AC-14 | ✅ |
| TC-977 | A language variant may not re-point WHICH case it is, by PRESENCE and not disagreement — and the forbidden list is READ from the declaration, so narrowing it narrows what is enforced (#242, quire-rs#342) | Unit | P0 | FR-043-AC-14 | ✅ |
| TC-978 | A corpus declaring no `case_schema.variant_forbidden` fails rather than enforcing nothing — a reader that enforces nothing when its rule is missing looks exactly like one that enforced it and found nothing (#242) | Unit | P0 | FR-043-AC-14 | ✅ |
