...
| NFR-011     | ✅ Covered | `tests/bundle-scale-performance.test.ts` — TC-290 generates 250 Markdown documents before timing, measures the owned completeness phase under 5s, and observes exactly one pass/250 read attempts. Separate 250-obligation workloads measure the advisor and auditor cores under 5s after the explicitly excluded Quire boundary. `matrix` is excluded because Quoin delegates it directly to `ix-flow`; timing that workflow here would attribute another component's work to Quoin. |
