...
| TC-116 | A payload omitting every optional key validates; one carrying every optional key validates, where "every" is read off the schema's own optional-key list so a refresh that adds a key fails until the fixture carries it (#178: the v0.41.0 refresh added `undeclared_statuses` and `implements` and the fixture silently narrowed); the v0.41.0 keys validate on their own; and a malformed entry is rejected — an `undeclared_statuses` row without its required `status`, and a malformed statement hash | Unit | P0 | FR-029-AC-7 | ✅ |
...
| TC-118 | A payload emitted by the **installed** `quire` binary validates against the vendored schema — the one thing a fixture cannot show. Covers **both** published contracts: `properties`, and `coverage` over a bundle that deliberately mints an `implements` edge, because a payload without one validates against the narrower schema too and the guard would prove nothing (CR-027) | Integration | P0 | FR-029-AC-9 | ✅ |
