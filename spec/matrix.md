...
| FR-102 | 🚧 Partially implemented | The native shell cutover is in `33ca665`: `quoin-cli` owns the shipped `quoin` binary; retained help fixtures, native grammar, `sync`, unknown-command refusal, and the no-Node-shell boundary are exercised in `tc_retained_help_capture.rs`, `sync.rs`, `tc_shell_contract.rs`, and TC-1654. `quoin-delivery` plus `native-release.yml` own GitHub Release delivery. Remaining: migrate the legacy `quoin-core` wire-only test population to the shipped command path, remove that development executable target, then perform a real tagged-release smoke after promotion and release authority. |
