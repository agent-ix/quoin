...
| FR-043-AC-14 | The runner reads a corpus case in **either layout the corpus declares** — a single `input/`, or a `<language>/input/` set whose shared `case.yaml` carries the metadata and whose per-language file carries only what varies — resolving the set the way the corpus's own readers do, with the language taken from the directory name and the id suffixed by it. A case authored into **both** layouts and a case authored into **neither** each fail the run naming the directory. A language variant declaring any field the corpus's `case_schema.variant_forbidden` names is rejected, and that list is **read from the declaration**: a corpus declaring none fails rather than enforcing nothing. | Test (TC-974, TC-975, TC-976, TC-977, TC-978) |
