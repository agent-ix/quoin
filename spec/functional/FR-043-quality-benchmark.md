...
| FR-043-AC-24 | `finding.actionability-v1` remains the historical row-id-or-line score. `finding.actionability-v2` separately counts applicable normalized findings carrying subject/locus, causal evidence, a change target, and a remedy or safe diagnostic step. It reports numerator, denominator, named unavailable misses, and named not-applicable exclusions, and has its own retained ratchet. | Test (TC-1082, TC-1083) |
...
| FR-043-AC-29 | A Tier-2 baseline retains raw and normalized output for every supported producer, including explicit unavailable and not-evaluated states; records the exact corpus and declaration revisions, executable digests and versions, canonical commands, and execution environment; and content-addresses the retained source set. Finding signals are scored from the retained `finding-envelope-v2` view, never by rereading parallel raw producer arrays; non-finding metric signals remain typed metrics. Candidate comparison is read-only, refuses unlike inputs, distinguishes expected unavailability from producer failure, and never replaces wild finding detection with controlled-family capability. Repeating the recorded commands over the recorded inputs yields identical retained source digests; changing a checkout root alone is outside that byte-identity claim because raw producer paths are evidence. | Test (TC-1101..TC-1104) |
