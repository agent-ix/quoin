...
`measured`, `out-of-model`, `unreadable`, or `unknown`, so that no document leaves the population
...
- When two or more resolved modules declare a document's frontmatter `type`, the measurement SHALL
  assign that document the state `unknown` carrying the competing module names.
- The measurement SHALL retain every `unknown` state in every published output.

The owner of a contested type is not determined by the data available, and guessing one would
fabricate the answer this measurement exists to obtain.
...
| FR-086-AC-4 | A `type` declared by two resolved modules yields `unknown` naming both modules, and no aggregate reassigns it. | Test (TC-1515) |
