...
| FR-113-AC-6 | Every ranked and unranked row carries a `label`: the plan's metric, followed by the row's dimension slice as `metric [k=v, …]` when the row has one, composed exactly as the per-repository report labels that row. The rendered text's ranking table and unranked list show it, and each `ranking.ranked[]` and `ranking.unranked[]` JSON entry carries it as `label` beside the bare `metric`, so two slices of one plan are distinguishable in both forms. The text form escapes `\`, `|`, CR and LF in the label so an authored dimension value cannot break the table row or bullet; the JSON `label` carries the authored bytes. | Test (TC-1926) |
