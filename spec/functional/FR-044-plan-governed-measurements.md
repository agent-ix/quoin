...
| FR-044-AC-9 | An observation's `population` may state `repetitions`, the number of repeated runs that produced it. A stated `population.repetitions` must be a whole number of at least 1, whatever the governing plan declares; any other stated value — a quoted number, a fraction, a negative, an object — is refused as `QM-POPULATION-MALFORMED`, naming the member and the value. When the governing plan declares `statistical_design.repetitions`, intake of a new collection refuses each `measured` observation whose stated `population.repetitions` is below it as `QM-REPETITIONS-SHORT`, naming the metric with its dimensions, the plan, the runs performed and the runs required; a count equal to the requirement is admitted. When the plan requires more than one repetition, a `measured` observation stating no `population.repetitions` is refused as `QM-POPULATION-UNSTATED`, naming that member; when it requires one, or declares no `statistical_design`, an unstated count is admitted. These findings accumulate as FR-044-AC-7 states. The JSON report carries a stored `repetitions` inside the observation's `population` exactly as stored, a non-numeric value included; a population without it states no such member. | Test (TC-1747..TC-1752, TC-1754, TC-1755) |
| FR-044-AC-10 | An observation may state an `interval` (engineering-assurance FR-021-AC-13). Intake of a new collection refuses, whatever the governing plan declares, a stated `interval` that does not deserialize as engineering-assurance's `Interval`, that has a `value` outside `[lower, upper]`, or that is on a `not_computed` observation, as `QM-INTERVAL-MALFORMED`, naming the metric with its dimensions, the member and the value. Quoin states no second copy of `Interval`'s own rules. There is no standard-error member: a producer states an interval. No `schemaVersion` changes: the member is optional, a reader that does not know it ignores it, and stored bytes are preserved. Findings accumulate as FR-044-AC-7 states. | Test (TC-1948, TC-1949, TC-1952) |
| FR-044-AC-11 | When the governing plan's `statistical_design.decision_rule` declares `interval_level`, intake of a new collection refuses each `measured` observation of the plan's own metric stating no `interval` as `QM-INTERVAL-UNSTATED`, and each stating an `interval` whose `level` is below the plan's `interval_level` as `QM-INTERVAL-LEVEL-SHORT`, each naming the metric with its dimensions, the plan, and the level stated and required; a `level` equal to or above the plan's is admitted. A stated `interval` that is malformed raises `QM-INTERVAL-MALFORMED` only, never the other two. A `not_computed` observation, and a `constant-predictor` item row (FR-108-AC-10, admitted under its governed metric's plan as one graded item, not a population), is not held to either. Under a plan declaring no `interval_level` an `interval` is admitted and no interval finding is raised for its absence. Quoin computes no interval for any estimator: for `proportion` and `count` too, a missing interval is refused and never derived from `population`. Findings accumulate as FR-044-AC-7 states. | Test (TC-1950, TC-1951, TC-1952) |
| FR-044-AC-12 | Every JSON view that carries an observation, the report and the series view included, carries the stored `interval` exactly as stored, a malformed retained value included, as an additive member omitted when the observation states none (the views are the report's `current` rows and the series view; the quoin-measurement-graph `GraphPartitionRow` is a fixed projection of state, value, unit, shape and reason that carries no `population` either, and does not carry `interval`), so each view over records with no `interval` is byte-identical to what it was before this criterion. | Test (TC-1953) |
