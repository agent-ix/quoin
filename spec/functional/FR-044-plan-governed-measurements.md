...
| FR-044-AC-9 | An observation's `population` may state `repetitions`, the number of repeated runs that produced it, read like `examined`. When the governing plan declares `statistical_design.repetitions`, intake of a new collection refuses each `measured` observation whose stated `population.repetitions` is below it as `QM-REPETITIONS-SHORT`, naming the metric with its dimensions, the plan, the runs performed and the runs required; a count equal to the requirement is admitted. When the plan requires more than one repetition, a `measured` observation stating no `population.repetitions` is refused as `QM-POPULATION-UNSTATED`, naming that member; when it requires one, or declares no `statistical_design`, an unstated count is admitted. These findings accumulate as FR-044-AC-7 states. The JSON report carries a stated `repetitions` inside the observation's `population`; a population without it states no such member. | Test (TC-1747..TC-1751) |
