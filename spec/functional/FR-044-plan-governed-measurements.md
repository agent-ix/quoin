...
| FR-044-AC-12 | Every JSON view that carries an observation, the report and the series view included, carries the stored `interval` exactly as stored, a malformed retained value included, as an additive member omitted when the observation states none, so each view over records with no `interval` is byte-identical to what it was before this criterion. | Test (TC-1953) |
