...
| FR-044-AC-10 | An observation may state an `interval`, an object of exactly `lower`, `upper`, `level` and `method` (engineering-assurance FR-021-AC-13's `Interval`): `lower`, `upper` and `level` are finite numbers, `lower` is at most the observation's `value` and `value` at most `upper` (same unit as `value`), `level` is strictly between 0 and 1, and `method` is non-empty text that no checker reads. Intake of a new collection refuses, whatever the governing plan declares, a stated `interval` that is not such an object, has a missing, extra, non-numeric or non-finite member, a `lower` above the `value` or a `value` above `upper`, a `level` outside 0 and 1 exclusive, or an empty or whitespace-only `method`, and any `interval` on a `not_computed` observation, as `QM-INTERVAL-MALFORMED`, naming the metric with its dimensions, the member and the value. There is no standard-error member: a producer states an interval. No `schemaVersion` changes: the member is optional, a reader that does not know it ignores it, and stored bytes are preserved. Findings accumulate as FR-044-AC-7 states. | Test (TC-1948, TC-1949, TC-1952) |
| FR-044-AC-11 | When the governing plan's `statistical_design.decision_rule` declares `interval_level` (engineering-assurance FR-021-AC-13), intake of a new collection refuses each `measured` observation stating no `interval` as `QM-INTERVAL-UNSTATED`, and each stating an `interval` whose `level` is below the plan's `interval_level` as `QM-INTERVAL-LEVEL-SHORT`, each naming the metric with its dimensions, the plan, and the level stated and required; a `level` equal to or above the plan's is admitted, and a `not_computed` observation is not held to either. Under a plan declaring no `interval_level` an `interval` is admitted and no interval finding is raised for its absence. Quoin does not compute or recompute an interval for any estimator, `proportion` and `count` included: the interval is taken as the producer reports it, as a `mean`, `median` or `ratio` `value` is. Findings accumulate as FR-044-AC-7 states. | Test (TC-1950, TC-1951, TC-1952) |
| FR-044-AC-12 | The JSON report carries an observation's stored `interval` exactly as stored, a malformed retained value included, as an additive member omitted when the observation states none, so a report over records with no `interval` is byte-identical to the report before this criterion. | Test (TC-1953) |
