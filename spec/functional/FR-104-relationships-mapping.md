...
- The bundle's package identity `<org>/<repo>` when the extracting surface knows it: the bundle package, else the package of an explicit `ix://<org>/<repo>/…` source identity, never the semantic module's package; and, when the extracting surface supplies a bundle index, its artifact `id`s with their object types; the package identities named in `semantic.imports`.
...
- If `semantic.mappings` names `relationships` and the `## Relationships` section holds no table, list, fence, or diagram, then Quire SHALL emit the warning `semantic.relationships-no-block` at the heading line with `reason` `no-block`. This departs from quire-rs FR-070 and FR-072, which report a prose-only section `unavailable` with reason `no-block`: a prose-only `## Relationships` section declares nothing, so there is nothing to extract, and the warning is enough.
...
- If `semantic.mappings` names `relationships`, the `## Relationships` section holds a table with the exact header, and the extracting surface supplies no relation vocabulary, then Quire SHALL report `availability.relations` `unavailable` with reason `no-relation-vocabulary`, emit no row diagnostic and no `relations`, and emit one advisory `semantic.relationships-no-vocabulary` at the section heading line with `reason` `no-relation-vocabulary`. A missing vocabulary is an explicit state, never a refusal per row; the `unavailable` state comes from the missing vocabulary, not from the advisory.
- If the relationships feature carries a shape error (a header, block, table, or section refusal of this requirement) and the surface supplies no relation vocabulary, then the shape error SHALL win: Quire SHALL report `availability.relations` `unavailable` with reason `entry-errors: lines <lines>` and SHALL NOT emit the `semantic.relationships-no-vocabulary` advisory.
- If `semantic.mappings` names `relationships`, the `## Relationships` section holds a table with the exact header, the surface supplies a relation vocabulary, and no bundle package is known, then Quire SHALL report `availability.relations` `unavailable` with reason `no-bundle-package`, emit no row diagnostic and no `relations`, and emit one advisory `semantic.relationships-no-bundle-package` at the section heading line with `reason` `no-bundle-package`. Quire SHALL NOT qualify targets under the semantic module's package, which names the module, not the artifact's bundle.
- If the relationships feature carries a shape error and no bundle package is known, then the shape error SHALL win: Quire SHALL report `availability.relations` `unavailable` with reason `entry-errors: lines <lines>` and SHALL NOT emit the `semantic.relationships-no-bundle-package` advisory.
...
- A target identity segment SHALL be in the semantic-core `SemanticId` id alphabet: an ASCII letter or digit, then ASCII letters, digits, `.`, `_`, `~`, `:`, or `-`. A `Target` whose id is outside that alphabet, such as a title with spaces, SHALL always be refused as `target-not-id`, with or without a bundle index. Without a bundle index, a title inside the alphabet (such as `ConfigOverlay`) cannot be told from an id and lowers with the `semantic.unresolved-target` advisory.
- If the extracting surface supplies no bundle index and a row's `Target` is a bare id in that alphabet other than the artifact's own `id`, or an `ix://<org>/<repo>/<id>` identity of the artifact's own package whose `<id>` is not the artifact's own `id`, then Quire SHALL emit the advisory `semantic.unresolved-target` at the row with `reason` `no-bundle-index` and a message naming the target, skip only the bundle-existence and `allowed_links` checks for that row, and lower the row with the target `ix://<org>/<repo>/<id>` under the artifact's own package.
- Quire SHALL emit the `semantic.unresolved-target` advisory only for a row that lowers. A row that fails another check SHALL yield exactly its one error diagnostic and no advisory.
- If any row of the table carries an error, then Quire SHALL emit no `semantic.unresolved-target` advisory for any row, including rows that passed every check: the feature is `unavailable` and no row lowers.
- The `semantic.unresolved-target` advisory SHALL NOT make `availability.relations` `unavailable` or lossy. The row is not lossy in the FR-072 sense: its target lowers to its real identity `ix://<org>/<repo>/<id>`, not a placeholder, so nothing in the row is left uninterpreted; only its existence in the bundle is unchecked.
...
| FR-104-AC-10 | `no-relation-vocabulary` (rows that would fail the verb and multiplicity checks) has exactly one diagnostic, the advisory `semantic.relationships-no-vocabulary` at the heading, no `relations`, and `availability.relations` `unavailable` with reason `no-relation-vocabulary`; `no-bundle-index` lowers both rows under `ix://agent-ix/config-service/`, with the advisory `semantic.unresolved-target` `reason` `no-bundle-index` naming `FR-005` at its row, no diagnostic for the artifact's own `FR-006`, and `availability.relations` `available`, not lossy; `no-bundle-index-title-target` (`Config Overlay`) is still `target-not-id`; `no-relation-vocabulary-second-table` has only `semantic.duplicate-section` with reason `entry-errors: lines 22` and no vocabulary advisory; `no-bundle-index-bad-multiplicity` has only the `multiplicity` error and no advisory; `no-bundle-index-own-package-identity` carries `ix://agent-ix/config-service/FR-005` verbatim with the `no-bundle-index` advisory. | Test |
| FR-104-AC-11 | `no-bundle-index-mixed-rows` (`references FR-005 1..1`, which passes its checks, and `references FR-006 many`) has exactly one diagnostic, the `multiplicity` error at line 21, no `semantic.unresolved-target` advisory, no `relations`, and `availability.relations` `unavailable` with reason `entry-errors: lines 21`. | Test |
| FR-104-AC-12 | `no-bundle-package` (rows that would fail the verb and multiplicity checks) has exactly one diagnostic, the advisory `semantic.relationships-no-bundle-package` at the heading with `reason` `no-bundle-package`, no `relations`, and `availability.relations` `unavailable` with reason `no-bundle-package`; `no-bundle-package-second-table` has only `semantic.duplicate-section` with reason `entry-errors: lines 22` and no bundle-package advisory. | Test |
