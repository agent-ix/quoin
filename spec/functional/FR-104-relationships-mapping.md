...
- If the extracting surface supplies no bundle index and a row's bare `Target` is not the artifact's own `id`, then Quire SHALL emit the advisory `semantic.unresolved-target` at the row with `reason` `no-bundle-index` and a message naming the target, skip the `target-not-id` and `target-not-allowed` checks for that row, and lower the row with the target `ix://<org>/<repo>/<Target>` under the artifact's own package, as quire-rs FR-070 maps an unresolved type with a `no-bundle-index` advisory.
