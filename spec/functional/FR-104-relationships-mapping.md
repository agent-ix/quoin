...
- A target identity segment SHALL be in the semantic-core `SemanticId` id alphabet: an ASCII letter or digit, then ASCII letters, digits, `.`, `_`, `~`, `:`, or `-`. A `Target` whose id is outside that alphabet, such as a title with spaces, SHALL always be refused as `target-not-id`, with or without a bundle index, because a title is never a target.
