...
outputs direct testing and specification work, so a plausible but incomplete
report has material impact. Required review covers the evidence, integrity,
and scope-boundary analyses, because changes can alter collection, scoring,
comparability, or what readers infer.

## Impact Scenarios

The material scenario is `concealed-measurement-gap`: a green summary is
accepted while a defect family has zero recall or most findings lack a repair
locus. Its consequence is repository work prioritized from incomplete or
unactionable evidence, across every repository and review consuming the report.
Detectability is low when gaps, provenance, and not-computed states are
aggregated away; the decision is reversible only after records and raw evidence
are recovered — invalidate the comparison, restore the raw record, and issue a
corrected report. Quoin turns tool observations into the information used for
QA decisions, and controlled-corpus scores do not establish precision on an
unadjudicated live repository.
